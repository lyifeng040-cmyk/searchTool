...
if IS_WINDOWS:
    try:
        ...
        if dll_path:
            ...
            # ★ 新增：预热和缓存管理函数
            RUST_ENGINE.warmup_dir_cache.argtypes = [ctypes.c_uint16]
            RUST_ENGINE.warmup_dir_cache.restype = ctypes.c_int32
            
            RUST_ENGINE.clear_dir_cache.argtypes = [ctypes.c_uint16]
            RUST_ENGINE.clear_dir_cache.restype = None
            
            RUST_ENGINE.clear_all_dir_cache.argtypes = []
            RUST_ENGINE.clear_all_dir_cache.restype = None
            ...
            with self.lock:
                ...
                if scope_targets:
                    for t in scope_targets:
                        ...
                        drv = Path(t_norm).drive.lower()  # 'g:' or ''
                        # 纯盘符目标：g: 或 g:\
                ...
                for fn, fp, sz, mt, is_dir in raw_results:
                    ...
                    path_drive = Path(path_lower).drive.lower()  # 'g:'
...
class DriveWarmupManager:
    ...
    def warmup_single(self, drive: str) -> bool:
        """预热单个驱动器（同步）"""
        ...
        try:
            ...
            result = RUST_ENGINE.warmup_dir_cache(ord(drive))
            elapsed = time.perf_counter() - start
            
            success = result == 1
            self._warmup_status[drive] = success
            
            if success:
                logger.info(f"🔥 驱动器 {drive}: 预热完成 ({elapsed:.2f}s)")
            else:
                logger.warning(f"❌ 驱动器 {drive}: 预热失败")
            
            return success
        except Exception as e:
            logger.error(f"❌ 驱动器 {drive}: 预热异常 - {e}")
            self._warmup_status[drive] = False
            return False
    
    def warmup_all_async(self, drives: list, callback=None):
        """异步并行预热所有驱动器"""
        if not HAS_RUST_ENGINE:
            if callback:
                callback({})
        ...
        def worker():
            results = {}
            sorted_drives = sorted(drives, key=lambda d: (d.upper().startswith('C'), d))
            ...
            for drive in sorted_drives:
                ...
                if len(drive_letter) == 1 and drive_letter.isalpha():
                    results[drive_letter] = self.warmup_single(drive_letter)
            
            if callback:
                callback(results)
        
        self._warmup_thread = threading.Thread(target=worker, daemon=True)
        self._warmup_thread.start()
    
    def is_warmed(self, drive: str) -> bool:
        """检查驱动器是否已预热"""
        return self._warmup_status.get(drive.upper().rstrip(":\\/"), False)
    ...
    def clear_cache(self, drive: str = None):
        ...
        try:
            if drive:
                drive = drive.upper().rstrip(":\\/")
                RUST_ENGINE.clear_dir_cache(ord(drive))
                self._warmup_status[drive] = False
                logger.info(f"🗑️ 已清除驱动器 {drive} 的缓存")
            else:
                RUST_ENGINE.clear_all_dir_cache()
                self._warmup_status.clear()
                logger.info("🗑️ 已清除所有驱动器缓存")
        except Exception as e:
    ...
    def stop(self):
        ...
        if self._warmup_thread and self._warmup_thread.is_alive():
            self._warmup_thread.join(timeout=2)


# 全局预热管理器实例
WARMUP_MGR = DriveWarmupManager()
...
class UsnFileWatcher(QObject):
    ...
    def _scan_dir_records(self, root_path, max_items=200000):
        """
...
class SearchApp(QMainWindow):
    ...
    def __init__(self, db_path=None):
        ...
        # ★ 只保留这一行，删除其他预热相关的 QTimer
        QTimer.singleShot(1000, self._maybe_start_warmup)
    ...
    def on_build_finished(self):
        ...
        # ★ 索引构建完成后重新预热
        QTimer.singleShot(500, self._start_warmup)
    ...
    def _build_menubar(self):
        ...
        tool_menu.addSeparator()
        # ★ 添加这两行
        tool_menu.addAction("🔥 重新预热缓存", self._restart_warmup)
        tool_menu.addAction("🗑️ 清除预热缓存", self._clear_warmup_cache)
        tool_menu.addSeparator()
    ...
    def _maybe_start_warmup(self):
        """检查是否需要预热"""
        if self.index_mgr.is_building:
            logger.info("⏸️ 索引构建中，跳过预热")
            return
        
        if self.index_mgr.file_count > 0:
            self._start_warmup()
        else:
            logger.info("⏸️ 索引为空，跳过预热")
    ...
    def _start_warmup(self):
        ...
        logger.info(f"🔥 开始预热驱动器缓存: {drives}")
        
        def on_warmup_complete(results):
            success_count = sum(1 for v in results.values() if v)
            total_count = len(results)
            logger.info(f"🔥 预热完成: {success_count}/{total_count} 个驱动器")
            
            if hasattr(self, 'status'):
                try:
                    self.status.setText(f"✅ 缓存预热完成 ({success_count}/{total_count})")
                except:
                    pass
        
        WARMUP_MGR.warmup_all_async(drives, callback=on_warmup_complete)

    def _restart_warmup(self):
        """重新预热缓存（模拟冷启动）"""
        WARMUP_MGR.stop()
        WARMUP_MGR.clear_cache()
        self.status.setText("🔥 正在重新预热...")
        # ★ 延迟更久，确保停止完成
        QTimer.singleShot(200, self._start_warmup)

    def _clear_warmup_cache(self):
        """清除预热缓存"""
        WARMUP_MGR.clear_cache()
        if hasattr(self, 'status'):
            self.status.setText("🗑️ 缓存已清除")
    ...
    def _render_page(self):
        ...
        # ===== 懒加载：仅补齐当前页文件 size/mtime，并写回数据库 =====
        try:
            tmp = []
            ...
            _batch_stat_files(
                ...
                only_missing=True,
                write_back_db=True,
                db_conn=self.index_mgr.conn,
                db_lock=self.index_mgr.lock,
            )
            ...
            for it, t in zip(page_items, tmp):
                ...
                if tc == 0:
                    ...
                    it["size_str"] = format_size(it["size"])

                it["mtime_str"] = format_time(it["mtime"])

        except Exception as e:
            logger.debug(f"页内懒加载补齐失败: {e}")
