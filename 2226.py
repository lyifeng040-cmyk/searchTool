    ...
    def build_index(self, drives, stop_fn=None):
        ...
        if HAS_RUST_ENGINE_V2:
            ...
            try:
                ...
                # 准备参数
                db_path = db_path_backup.encode('utf-8')
                drives_str = ','.join([d.rstrip(':\\').upper() for d in drives if os.path.exists(d)]).encode('utf-8')
                c_paths = ','.join(get_c_scan_dirs(self.config_mgr)).encode('utf-8')
                
                # 调用 Rust 引擎
                result = RUST_ENGINE.scan_and_index(db_path, drives_str, c_paths, None)
                ...
                if result.success:
                    ...
                    logger.info(f"✅ Rust V2 索引构建完成: {self.file_count:,} 条, 耗时 {elapsed:.2f}s")
                    self.progress_signal.emit(self.file_count, f"完成: {self.file_count:,} 条")
                else:
                    error_msg = result.error_msg.decode('utf-8') if result.error_msg else "未知错误"
                    logger.error(f"❌ Rust V2 引擎失败: {error_msg}")
                    if result.error_msg:
    ...
    def rebuild_drive(self, drive_letter, progress_callback=None, stop_fn=None):
        ...
        try:
            ...
            if HAS_RUST_ENGINE_V2:
                ...
                result = RUST_ENGINE.rebuild_drive_index(db_path, drive_char, c_paths, None)
                
                # ★★★ 关键：重新打开数据库连接 ★★★
                self._reopen_connection()
                
                if result.success:
                    logger.info(f"✅ {drive}: 盘索引重建完成，写入 {result.file_count} 条记录")
                    self._load_stats(preserve_mft=True)
                    self.is_ready = self.file_count > 0
                else:
                    error_msg = result.error_msg.decode('utf-8') if result.error_msg else "未知错误"
                    logger.error(f"❌ Rust V2 重建失败: {error_msg}")
                    if result.error_msg:
                        RUST_ENGINE.free_error_msg(result.error_msg)
                    # 回退到旧方法
                    self._rebuild_drive_fallback(drive, progress_callback, stop_fn)
