...
class MiniSearchWindow(QObject):
	...
	def _create_window(self):
		...
		self.window.setStyleSheet(
			...
			QLineEdit { padding: 10px; font-size: 15px; border: 2px solid #88c0d8; border-radius: 4px; background: white; }
			QLineEdit:focus { border-color: #006699; }
			QListWidget { background: white; border: 1px solid #88c0d8; font-size: 12px; color: #0f2f3c; selection-background-color: #0c6c8c; selection-color: #ffffff; }
			QListWidget::item { padding: 7px 8px; }
			QListWidget::item:hover { background-color: #e6f5fb; }
			QListWidget::item:selected { background-color: #0c6c8c; color: #ffffff; }
			QListWidget::item:selected:!active { background-color: #4fa3c7; color: #ffffff; }
			QListWidget::item:selected:hover { background-color: #0a5d7a; color: #ffffff; }
			QPushButton { padding: 7px 12px; background: white; border: 1px groove #ccc; border-radius: 4px; font-size: 11px; color: #004466; }
		...
		self.search_icon.setFont(QFont("Segoe UI Emoji", 20))
		...
		self.search_entry.setFont(QFont("微软雅黑", 15))
	...
	def _search_index(self, keyword):
		if not self.app.index_mgr.is_ready:
			self.result_listbox.addItem(QListWidgetItem("   ⚠️ 索引未就绪，请先构建索引"))
			return
