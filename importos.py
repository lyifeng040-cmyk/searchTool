...
from PySide6.QtWidgets import (
    ...
    QFileDialog, QFrame, QSystemTrayIcon, QHeaderView, QAbstractItemView,
    QGroupBox, QScrollArea, QTabWidget, QTextEdit, QSpinBox, QRadioButton,
    QGridLayout, QSizePolicy, QInputDialog
)
from PySide6.QtCore import (
    Qt, QThread, Signal, QTimer, QSize, QUrl, QEvent, QObject
)
from PySide6.QtGui import (
    QAction, QIcon, QFont, QColor, QDesktopServices,
    QKeySequence, QShortcut, QPixmap, QPainter
)
    ...
    def enum_volume_files_mft(drive_letter, skip_dirs, skip_exts, allowed_paths=None):
        ...
        if HAS_RUST_ENGINE:
            ...
            try:
                ...
                while off < n:
                    ...
                    name_lower_len = int.from_bytes(
                        raw_data[off + 3 : off + 5], "little"
                    )
                    ...
                    total_len = (
                        name_len + name_lower_len + path_len + parent_len + ext_len
                    )
                    if off + total_len > n:
                        break
                    ...
                    off += name_len
                    name_lower = raw_data[off : off + name_lower_len].decode(
                        "utf-8", "replace"
                    )
                    off += name_lower_len
                    ...
                    off += parent_len
                    ext = (
                        raw_data[off : off + ext_len].decode("utf-8", "replace")
                        if ext_len
                        else ""
                    )
                    off += ext_len
                ...
                logger.info(
                    f"过滤: Rust返回={result.count}, 跳过={skipped_count}, 保留={len(py_list)}"
                )
                ...
                if files_to_stat:
                        ...
                        for item in items:
                            try:
                                ...
                                if GetFileAttributesExW(item[2], 0, ctypes.byref(data)):
                                    ...
                                    mtime_ft = (
                                        data.ftLastWriteTime.dwHighDateTime << 32
                    ...
                    with concurrent.futures.ThreadPoolExecutor(
                        ...
                        for i in range(0, len(files_to_stat), batch_size):
                            futures.append(
                                executor.submit(
                                    stat_batch, files_to_stat[i : i + batch_size]
                                )
                            )
        ...
        try:
            ...
            for ref, (name, parent_ref) in files.items():
                ...
                result.append(
                    [name, name.lower(), full_path, parent_path, ext, 0, 0, 0]
                )
            ...
            if files_to_stat:
                logger.info(
                    f"[MFT] {drive}: 开始获取 {len(files_to_stat):,} 个文件的大小和时间..."
                )
                    ...
                    for item in items_batch:
                        try:
                            ...
                            if GetFileAttributesExW(
                                item[2], GET_FILEEX_INFO_LEVELS, ctypes.byref(data)
                            ):
...
class IndexManager(QObject):
    ...
    def __init__(self, db_path=None, config_mgr=None):
        ...
        if db_path is None:
            idx_dir = LOG_DIR; idx_dir.mkdir(exist_ok=True)
            self.db_path = str(idx_dir / "index.db")
        else:
            self.db_path = db_path
        self.conn = None; self.lock = threading.RLock(); self.is_ready = False; self.is_building = False; self.file_count = 0; self.last_build_time = None; self.has_fts = False; self.used_mft = False
        self._init_db()

    def _init_db(self):
        try:
            if HAS_APSW: self.conn = apsw.Connection(self.db_path)
            else: self.conn = sqlite3.connect(self.db_path, check_same_thread=False)
            cursor = self.conn.cursor(); cursor.execute("PRAGMA journal_mode=WAL"); cursor.execute("PRAGMA synchronous=NORMAL"); cursor.execute("PRAGMA cache_size=-2000000"); cursor.execute("PRAGMA temp_store=MEMORY")
            cursor.execute("""CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY, filename TEXT NOT NULL, filename_lower TEXT NOT NULL, full_path TEXT UNIQUE NOT NULL, parent_dir TEXT NOT NULL, extension TEXT, size INTEGER DEFAULT 0, mtime REAL DEFAULT 0, is_dir INTEGER DEFAULT 0)""")
            try:
                fts_exists = False
                for row in cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='files_fts'"): fts_exists = True; break
                if not fts_exists:
                ...
                self.has_fts = True; logger.info("✅ FTS5 已启用")
            except Exception as e: self.has_fts = False; logger.warning(f"⚠️ FTS5 不可用: {e}")
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_fn ON files(filename_lower)"); cursor.execute("CREATE INDEX IF NOT EXISTS idx_parent ON files(parent_dir)"); cursor.execute("CREATE TABLE IF NOT EXISTS meta (key TEXT PRIMARY KEY, value TEXT)")
            if not HAS_APSW: self.conn.commit()
            self._load_stats()
        except Exception as e: logger.error(f"❌ 数据库初始化错误: {e}"); self.conn = None
    ...
    def _load_stats(self, preserve_mft=False):
        ...
        try:
            with self.lock:
                ...
                count_result = list(cursor.execute("SELECT COUNT(*) FROM files")); self.file_count = count_result[0][0] if count_result else 0
                time_row = list(cursor.execute("SELECT value FROM meta WHERE key='build_time'"))
                if time_row and time_row[0][0]:
                    try: self.last_build_time = float(time_row[0][0])
                    except (ValueError, TypeError): self.last_build_time = None
                else: self.last_build_time = None
                # 始终从数据库读取MFT状态
                mft_row = list(cursor.execute("SELECT value FROM meta WHERE key='used_mft'")); self.used_mft = bool(mft_row and mft_row[0][0] == "1")
            self.is_ready = self.file_count > 0
        except Exception as e: logger.error(f"加载统计信息失败: {e}"); self.file_count = 0; self.is_ready = False

    def reload_stats(self):
        if not self.is_building: self._load_stats(preserve_mft=True)
    ...
    def close(self):
        with self.lock:
            if self.conn:
                try: self.conn.close(); logger.info("数据库连接已关闭")
                except Exception as e: logger.warning(f"关闭数据库连接失败: {e}")
                finally: self.conn = None

    def change_db_path(self, new_path):
        if not new_path: return False, "路径不能为空"
        new_path = os.path.abspath(new_path)
        try: os.makedirs(os.path.dirname(new_path), exist_ok=True)
        except OSError as e: logger.error(f"创建目录失败: {e}")
        self.close()
        ...
        self.db_path = new_path; self.conn = None; self._init_db(); return True, "已更改"

    def search(self, keywords, scope_targets, limit=50000):
        if not self.conn or not self.is_ready: return None
        try:
            with self.lock:
                cursor = self.conn.cursor()
                wheres = " AND ".join(["filename_lower LIKE ?"] * len(keywords)); sql = f"SELECT filename, full_path, size, mtime, is_dir FROM files WHERE {wheres} LIMIT ?"; params = tuple([f"%{kw}%" for kw in keywords] + [limit])
                raw_results = list(cursor.execute(sql, params))
                filtered = []
                scope_targets_lower = [os.path.normpath(t).lower() for t in scope_targets] if scope_targets else None
                for row in raw_results:
                    path_lower = os.path.normpath(row[1]).lower()
                    if scope_targets_lower:
                        path_drive = Path(path_lower).drive.lower().rstrip('\\')
                        if path_drive in scope_targets_lower: pass
                        elif any(path_lower.startswith(allowed) for allowed in scope_targets_lower): pass
                        else: continue
                    if should_skip_path(path_lower): continue
                    name_lower = row[0].lower()
                    if row[4]:
                        if should_skip_dir(name_lower, path_lower): continue
                    else:
                        if os.path.splitext(name_lower)[1] in SKIP_EXTS: continue
                    filtered.append(row)
                return filtered
        except Exception as e: logger.error(f"搜索错误: {e}"); return None

    def get_stats(self):
        self._load_stats(preserve_mft=True)
        return {"count": self.file_count, "ready": self.is_ready, "building": self.is_building, "time": self.last_build_time, "path": self.db_path, "has_fts": self.has_fts, "used_mft": self.used_mft}

    def build_index(self, drives, stop_fn=None):
        global MFT_AVAILABLE
        if not self.conn or self.is_building: return
        self.is_building = True; self.is_ready = False; self.used_mft = False; MFT_AVAILABLE = False; build_start = time.time()
        try:
            logger.info("🚀 开始构建索引..."); self.progress_signal.emit(0, "阶段1/5: 清理旧数据...")
            with self.lock:
                cursor = self.conn.cursor(); cursor.execute("DROP TRIGGER IF EXISTS files_ai"); cursor.execute("DROP TRIGGER IF EXISTS files_ad"); cursor.execute("DROP TABLE IF EXISTS files_fts"); cursor.execute("DROP TABLE IF EXISTS files")
                cursor.execute("""CREATE TABLE files (id INTEGER PRIMARY KEY, filename TEXT NOT NULL, filename_lower TEXT NOT NULL, full_path TEXT UNIQUE NOT NULL, parent_dir TEXT NOT NULL, extension TEXT, size INTEGER DEFAULT 0, mtime REAL DEFAULT 0, is_dir INTEGER DEFAULT 0)""")
                if not HAS_APSW: self.conn.commit()
                self.has_fts = False; self.file_count = 0
            logger.info(f"✅ 阶段1完成: {time.time() - build_start:.2f}s")
            self.progress_signal.emit(0, "阶段2/5: MFT扫描...")
            all_drives = [d.upper().rstrip(":\\") for d in drives if os.path.exists(d)]; c_allowed_paths = get_c_scan_dirs(self.config_mgr); all_data, failed_drives = [], []
            if all_drives and IS_WINDOWS:
                data_lock = threading.Lock()
                def scan_one(drv):
                    try:
                        allowed = c_allowed_paths if drv == 'C' else None; data = enum_volume_files_mft(drv, SKIP_DIRS_LOWER, SKIP_EXTS, allowed_paths=allowed)
                        with data_lock: all_data.extend(data)
                        return drv, len(data)
                    except Exception as e: logger.error(f"扫描驱动器 {drv} 失败: {e}"); return drv, -1
                with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(all_drives), 4)) as ex:
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        ...
                        self.progress_signal.emit(len(all_data), f"MFT {drv}: {result if result >= 0 else '失败'}")
                if all_data: self.used_mft = True
            logger.info(f"✅ 阶段2完成: {time.time() - build_start:.2f}s, 扫描到 {len(all_data):,} 条")
            if all_data:
                self.progress_signal.emit(len(all_data), "阶段3/5: 写入数据库...")
                with self.lock: cursor = self.conn.cursor(); cursor.execute("PRAGMA synchronous=OFF"); cursor.execute("PRAGMA journal_mode=MEMORY")
                batch_size, total_written = 50000, 0
                for i in range(0, len(all_data), batch_size):
                    ...
                    try:
                        with self.lock:
                            cursor = self.conn.cursor()
                            if HAS_APSW:
                                with self.conn: cursor.executemany("INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)", batch)
                            else: cursor.executemany("INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)", batch); self.conn.commit()
                        total_written += len(batch); self.progress_signal.emit(total_written, f"写入: {total_written:,}/{len(all_data):,}")
                    except Exception as e: logger.error(f"写入批次失败: {e}")
                self.file_count = total_written
                logger.info(f"✅ 阶段3完成: {time.time() - build_start:.2f}s, 写入 {total_written:,} 条")
            self.progress_signal.emit(self.file_count, "阶段4/5: 创建索引...")
            with self.lock:
                cursor = self.conn.cursor(); cursor.execute("CREATE INDEX IF NOT EXISTS idx_fn ON files(filename_lower)"); cursor.execute("CREATE INDEX IF NOT EXISTS idx_parent ON files(parent_dir)"); cursor.execute("PRAGMA synchronous=NORMAL"); cursor.execute("PRAGMA journal_mode=WAL")
                if not HAS_APSW: self.conn.commit()
            logger.info(f"✅ 阶段4完成: {time.time() - build_start:.2f}s")
            self.progress_signal.emit(self.file_count, "阶段5/5: 构建全文索引(后台)...")
            def build_fts_async():
                try:
                    logger.info("📝 后台构建 FTS5..."); fts_start = time.time()
                    with self.lock:
                        cursor = self.conn.cursor(); cursor.execute("CREATE VIRTUAL TABLE IF NOT EXISTS files_fts USING fts5(filename, content=files, content_rowid=id)"); cursor.execute("INSERT INTO files_fts(files_fts) VALUES('rebuild')")
                        cursor.execute("CREATE TRIGGER IF NOT EXISTS files_ai AFTER INSERT ON files BEGIN INSERT INTO files_fts(rowid, filename) VALUES (new.id, new.filename); END")
                        cursor.execute("CREATE TRIGGER IF NOT EXISTS files_ad AFTER DELETE ON files BEGIN INSERT INTO files_fts(files_fts, rowid, filename) VALUES('delete', old.id, old.filename); END")
                        if not HAS_APSW: self.conn.commit()
                        self.has_fts = True
                    logger.info(f"✅ FTS5 构建完成: {time.time() - fts_start:.2f}s")
                except Exception as e: logger.warning(f"⚠️ FTS5 构建失败: {e}"); self.has_fts = False
            ...
            for drv in failed_drives:
                ...
                for path in paths_to_scan: logger.info(f"[传统扫描] {path}"); self._scan_dir(path, c_allowed_paths if drv == 'C' else None, stop_fn)
            try:
                with self.lock: cursor = self.conn.cursor(); final_count = list(cursor.execute("SELECT COUNT(*) FROM files"))[0][0]; self.file_count = final_count
            except: pass
            logger.info(f"✅ 索引构建完成: {self.file_count:,} 条, 耗时 {time.time() - build_start:.2f}s"); self.is_ready = self.file_count > 0
            self.build_finished_signal.emit()
        except Exception as e: import traceback; logger.error(f"❌ 构建错误: {e}"); traceback.print_exc()
        finally: self.is_building = False

    def _scan_dir(self, target, allowed_paths=None, stop_fn=None):
        try:
            if not os.path.exists(target): return
        except (OSError, PermissionError): logger.warning(f"无法访问目录: {target}"); return
        allowed_paths_lower = [p.lower().rstrip("\\") for p in allowed_paths] if allowed_paths else None; batch, stack = [], deque([target])
        while stack:
            if stop_fn and stop_fn(): break
            cur = stack.pop()
            ...
            try:
                with os.scandir(cur) as it:
                    for e in it:
                        ...
                        try: is_dir = e.is_dir(); st = e.stat(follow_symlinks=False)
                        except (OSError, PermissionError): continue
                        path_lower = e.path.lower()
                        if is_dir:
                            if should_skip_dir(e.name.lower(), path_lower, allowed_paths_lower): continue
                            stack.append(e.path); batch.append((e.name, e.name.lower(), e.path, cur, "", 0, 0, 1))
                        else:
                        ...
                        if len(batch) >= 20000:
                            with self.lock:
                                cursor = self.conn.cursor(); cursor.executemany("INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)", batch)
                                if not HAS_APSW: self.conn.commit()
                            self.file_count += len(batch); self.progress_signal.emit(self.file_count, cur); batch = []
    ...
    def _apply(self, events):
            ...
            if ev[0] == "c":
                ...
                try:
                    if os.path.isfile(p):
                        ...
                        logger.debug(f"[监控] 新增文件: {p}")
                except (FileNotFoundError, PermissionError, OSError) as e:
                    logger.debug(f"监控新增失败 {p}: {e}")
            elif ev[0] == "d":
                dels.append(ev[1])
                logger.debug(f"[监控] 删除文件: {ev[1]}")
            elif ev[0] == "m":
                dels.append(ev[1])
                p = ev[2]
                try:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"[监控] 数据库已更新: +{len(ins)} -{len(dels)}")
                break

            except Exception as e:
                if attempt < max_retries - 1:
                    logger.warning(f"数据库忙，重试 {attempt + 1}/{max_retries}")
                    time.sleep(0.5)
                else:
...
class TrayManager:
    ...
    def _refresh_status(self):
        QTimer.singleShot(0, self.app.refresh_index_status)
...
class MiniSearchWindow(QObject):
    ...
    def _create_window(self):
        ...
        self.left_arrow.enterEvent = lambda e: self.left_arrow.setStyleSheet("color: #0088cc;")
        self.left_arrow.leaveEvent = lambda e: self.left_arrow.setStyleSheet("color: #004466;")
        ...
        self.right_arrow.enterEvent = lambda e: self.right_arrow.setStyleSheet("color: #0088cc;")
        self.right_arrow.leaveEvent = lambda e: self.right_arrow.setStyleSheet("color: #004466;")
        ...
        self.btn_to_main.setFont(QFont("微软雅黑", 9))
    ...
    def _on_switch_to_main(self, event=None):
        ...
        if keyword:
            self.app.kw_var.setText(keyword)
...
class CDriveSettingsDialog:
    ...
    def _build_ui(self):
        ...
        # 说明文字
        desc_label = QLabel("设置C盘索引扫描的目录范围，勾选启用，取消勾选禁用，点击 ✕ 删除")
        ...
        self.scroll_area.setStyleSheet("QScrollArea { background-color: #fafafa; border: 1px solid #ddd; }")
...
class SearchApp(QMainWindow):
    def __init__(self, db_path=None):
        ...
        self.setWindowTitle("🚀 极速文件搜索 V42 增强版 (PySide6 UI)")
    ...
    def on_build_finished(self):
        self.index_mgr.force_reload_stats() # 强制从DB加载最终计数值
        self._check_index() # 更新UI
    ...
    def on_fts_finished(self):
        logger.info("接收到 FTS_DONE 信号，准备刷新UI状态。")
    ...
    def _build_menubar(self):
        ...
        # 这里只是菜单勾选状态，逻辑仍然用 self.fuzzy_var/self.regex_var
        self.fuzzy_action = search_menu.addAction("模糊搜索")
        self.fuzzy_action.setCheckable(True)
        self.fuzzy_action.setChecked(True)
        self.fuzzy_action.triggered.connect(self._toggle_fuzzy)

        self.regex_action = search_menu.addAction("正则表达式")
        self.regex_action.setCheckable(True)
        self.regex_action.setChecked(False)
        self.regex_action.triggered.connect(self._toggle_regex)
        ...
        tool_menu.addAction("🔍 查找重复文件", self.find_duplicates)
        tool_menu.addAction("📁 查找空文件夹", self.find_empty_folders)
    ...
    def _toggle_fuzzy(self, checked):
        self.fuzzy_var = checked

    def _toggle_regex(self, checked):
        self.regex_var = checked
    ...
    def _build_ui(self):
        ...
        btn_refresh_idx = QPushButton("🔄 刷新状态")
        btn_refresh_idx.setFixedWidth(90)
        btn_refresh_idx.clicked.connect(self.refresh_index_status)
        ...
        # 关键字输入
        self.kw_var = QLineEdit()
        self.entry_kw = self.kw_var
    ...
    def _bind_shortcuts(self):
        ...
        QShortcut(QKeySequence("Escape"), self, lambda: self.stop_search() if self.is_searching else self.kw_var.clear())
    ...
    def _show_settings(self):
        ...
        hotkey_var = self.config_mgr.get_hotkey_enabled()
        self.chk_hotkey = QCheckBox("启用全局热键 (Ctrl+Shift+Space)")
        self.chk_hotkey.setChecked(hotkey_var)
        ...
        tray_var = self.config_mgr.get_tray_enabled()
        self.chk_tray = QCheckBox("关闭时最小化到托盘")
        self.chk_tray.setChecked(tray_var)
    ...
    def _update_drives(self):
        if platform.system() == "Windows":
            ...
            self.combo_scope.addItems(drives)
        else:
            self.combo_scope.clear()
            self.combo_scope.addItems(["所有磁盘 (全盘)", "/"])
...
全局热键(需启用):
  ...
  Ctrl+Shift+Tab    主窗口
    ...
    def _apply_filter(self):
        ...
        current_filter_text = self.lbl_filter.text()
        if "磁盘筛选" in current_filter_text:
            base_text = current_filter_text.split(" | ")[0]
            if ext_sel != "全部" or size_min > 0 or date_min > 0:
                self.lbl_filter.setText(f"{base_text} | 筛选: {filtered_count}/{all_count}")
            else:
                self.lbl_filter.setText(base_text)
        else:
            if ext_sel != "全部" or size_min > 0 or date_min > 0:
    ...
    def _render_page(self):
        ...
        for i, item in enumerate(page_items):
            ...
            # 核心修改：使用我们自己的 SortableTreeWidgetItem
            q_item = SortableTreeWidgetItem(row_data)
            
            # 依然需要设置 UserRole 数据，供 __lt__ 方法使用
            q_item.setData(2, Qt.UserRole, item["size"])
            q_item.setData(3, Qt.UserRole, item["mtime"])
            
            # 行背景色
            bg_color = QColor("#ffffff") if i % 2 == 0 else QColor("#f8f9fa")
            for col in range(self.tree.columnCount()):
                q_item.setBackground(col, bg_color)

            items_to_add.append(q_item)
            self.item_meta[id(q_item)] = start + i
        
        # 使用 addTopLevelItems 批量添加，性能更好
    ...
    def delete_file(self):
        ...
        if failed:
            ...
            self.status.setText(f"✅ 已删除 {deleted} 个文件")
    ...
    def refresh_search(self):
        if self.last_search_params and not self.is_searching:
            self.kw_var.setText(self.last_search_params["kw"])
            self.start_search()

    def toggle_pause(self):
        if not self.is_searching or not hasattr(self.worker, 'toggle_pause'):
            return
    ...
    def _reset_ui(self):
        ...
        self.btn_pause.setEnabled(False); self.btn_pause.setText("⏸ 暂停")
        self.btn_stop.setEnabled(False)
        self.progress.setVisible(False)
    ...
    def on_batch_ready(self, batch):
        with self.results_lock:
            for item_data in batch:
                if item_data['fullpath'] not in self.shown_paths:
                    self.shown_paths.add(item_data['fullpath'])
    ...
    def on_rt_progress(self, scanned_dirs, speed):
        self.status.setText(f"🔍 实时扫描中... (已扫描 {scanned_dirs:,} 个目录，{speed:.0f}/s)")
    ...
    def sort_column(self, logical_index):
        """
        响应表头点击事件，对 QTreeWidget 中的项进行排序。
        这个版本包含了对数字列的特殊处理。
        """
        self.tree.sortItems(logical_index, self.tree.header().sortIndicatorOrder())

        # 获取当前的排序列和顺序
        current_column = self.tree.sortColumn()
        current_order = self.tree.header().sortIndicatorOrder()

        # 如果点击的是同一列，则切换顺序；否则，默认升序
        if logical_index == current_column:
            new_order = Qt.DescendingOrder if current_order == Qt.AscendingOrder else Qt.AscendingOrder
        else:
            new_order = Qt.AscendingOrder
        
        # 核心逻辑：告诉 QTreeWidget 如何排序
        # 对于大小(列2)和时间(列3)，我们使用之前在 _render_page 中设置的 Qt.UserRole 数据
        # 对于其他列，使用默认的文本排序 (Qt.DisplayRole)
        if logical_index in [2, 3]:
            self.tree.sortItems(logical_index, new_order)
        else:
            # 对于文本列，默认行为就是正确的
            self.tree.sortItems(logical_index, new_order)

        self.tree.setSortingEnabled(False) # 完成后最好禁用

        # 更新表头的排序箭头
        self.tree.header().setSortIndicator(logical_index, new_order)
    ...
    def _build_index(self):
        ...
        # 使用线程直接调用 build_index，不再需要 run 函数和 queue
        threading.Thread(
            target=self.index_mgr.build_index,
            args=(self._get_drives(), lambda: self.index_build_stop),
            daemon=True
        ).start()
    ...
    def export_results(self):
        ...
        try:
            ...
            self.status.setText(f"✅ 已导出 {len(self.all_results)} 条结果到: {os.path.basename(path)}")
    ...
    def find_duplicates(self):
        """查找重复文件"""
        QMessageBox.information(self, "提示", "查找重复文件功能开发中...\n\n将基于文件大小和MD5哈希值查找重复文件。")

    # ==================== 查找空文件夹 ====================
    def find_empty_folders(self):
        """查找空文件夹"""
        QMessageBox.information(self, "提示", "查找空文件夹功能开发中...\n\n将扫描指定路径下的所有空文件夹。")
...
class IndexSearchWorker(QThread):
    ...
    def _match(self, filename):
        if self.regex_mode:
            try: return re.search(self.keyword_str, filename, re.IGNORECASE)
    ...
    def run(self):
        ...
        try:
            ...
            results = self.index_mgr.search(search_keywords, self.scope_targets)
            if results is None:
            ...
            for fn, fp, sz, mt, is_dir in results:
                ...
                if not self._match(fn): continue

                ext = os.path.splitext(fn)[1].lower()
                tc = 0 if is_dir else (1 if ext in ARCHIVE_EXTS else 2)
                batch.append({
                    'filename': fn, 'fullpath': fp, 'dir_path': os.path.dirname(fp),
                    'size': sz, 'mtime': mt, 'type_code': tc,
                    'size_str': "📂 文件夹" if tc == 0 else ("📦 压缩包" if tc == 1 else format_size(sz)),
                    'mtime_str': format_time(mt)
                })
                if len(batch) >= 200:
                    self.batch_ready.emit(list(batch)); batch.clear()
            
            if batch: self.batch_ready.emit(batch)
            self.finished.emit(time.time() - start_time)
        except Exception as e:
            logger.error(f"索引搜索线程错误: {e}"); self.error.emit(str(e))
...
class RealtimeSearchWorker(QThread):
    ...
    def toggle_pause(self, paused): self.is_paused = paused
    ...
    def run(self):
        ...
        try:
            ...
            for t in self.scope_targets:
                if os.path.isdir(t): task_queue.put(t)
            
            active_threads = [0]; lock = threading.Lock(); scanned_dirs = [0]
            ...
            def worker():
                ...
                while not self.stopped:
                    while self.is_paused:
                        if self.stopped: return
                        time.sleep(0.1)
                    try: cur = task_queue.get(timeout=0.1)
                    except queue.Empty:
                        with lock:
                            if task_queue.empty() and active_threads[0] <= 1: break
                        continue
                    
                    with lock: active_threads[0] += 1; scanned_dirs[0] += 1
                        ...
                        with os.scandir(cur) as it:
                            for e in it:
                                ...
                                if self._match(e.name):
                                    ...
                                    tc = 0 if is_dir else (1 if ext in ARCHIVE_EXTS else 2)
                                    local_batch.append({
                                        'filename': e.name, 'fullpath': e.path, 'dir_path': cur,
                                        'size': st.st_size, 'mtime': st.st_mtime, 'type_code': tc,
                                        'size_str': "📂 文件夹" if tc == 0 else ("📦 压缩包" if tc == 1 else format_size(st.st_size)),
                                ...
                                if len(local_batch) >= 50:
                                    self.batch_ready.emit(list(local_batch)); local_batch.clear()
                                    elapsed = time.time() - start_time
                                    speed = scanned_dirs[0] / elapsed if elapsed > 0 else 0
                                    self.progress.emit(scanned_dirs[0], speed)
                    except (PermissionError, OSError): pass
                    with lock: active_threads[0] -= 1
                if local_batch: self.batch_ready.emit(local_batch)

            threads = [threading.Thread(target=worker, daemon=True) for _ in range(16)]
            for t in threads: t.start()
            for t in threads: t.join()
            
            if not self.stopped: self.finished.emit(time.time() - start_time)
        except Exception as e:
            logger.error(f"实时搜索线程错误: {e}"); self.error.emit(str(e))

class SortableTreeWidgetItem(QTreeWidgetItem):
    """
    一个可排序的 QTreeWidgetItem。
    重写 __lt__ 方法，以便在排序时能正确比较数字。
    """
    def __lt__(self, other):
        # 获取当前正在排序的列
        column = self.treeWidget().sortColumn()
        
        # 如果是大小列(2)或时间列(3)，则按数字比较
        if column in [2, 3]:
            # 从 Qt.UserRole 中获取我们之前存储的原始数字值
...
def main():
    ...
    logger.info("新增功能: C盘目录设置、磁盘筛选联动、全局热键、系统托盘 (逻辑同原版)")
    ...
    app.setStyleSheet("""
        QHeaderView::section {
            ...
            padding: 4px;
            border: 1px solid #dcdcdc; /* 设置清晰的边框 */
            border-left: none; /* 避免双边框 */
            font-weight: bold;
        }
        
        QHeaderView::section:first {
            border-left: 1px solid #dcdcdc; /* 为第一列加上左边框 */
        }

        QTreeWidget {
            border: 1px solid #dcdcdc; /* 给整个表格控件也加上边框 */
        }
    """)
