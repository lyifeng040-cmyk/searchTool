...
from cpp_scanner_wrapper import CppMFTScanner
    ...
    def enum_volume_files_mft(drive_letter, skip_dirs, skip_exts, allowed_paths=None):
        ...
        try:
            ...
            if not DeviceIoControl(h, FSCTL_QUERY_USN_JOURNAL, None, 0, ctypes.byref(jd), ctypes.sizeof(jd), ctypes.byref(br), None):
                error_code = ctypes.get_last_error()
                logger.error(f"查询USN失败 {drive}: ���误代码 {error_code}")
            ...
            for ref, (name, parent, is_dir) in records.items():
                ...
                if allowed_paths_lower:
                    in_scope = is_in_allowed_paths(path_lower, allowed_paths_lower)
                    if not in_scope:
                        continue
                
                in_allowed = allowed_paths_lower and is_in_allowed_paths(path_lower, allowed_paths_lower)
                ...
                if should_skip_path(path_lower, allowed_paths_lower):
                    continue
                
                if is_dir:
                    if should_skip_dir(name_lower, path_lower, allowed_paths_lower):
                        continue
                else:
...
class IndexManager:
    def __init__(self, db_path=None, config_mgr=None):
        ...
        try:
            from cpp_scanner_wrapper import CppMFTScanner
            self.cpp_scanner = CppMFTScanner()
            logger.info("✅ 已加载 C++ MFT 扫描器")
        except Exception as e:
            logger.warning(f"⚠️ 加载 C++ 扫描器失败，将使用 Python MFT: {e}")
            self.cpp_scanner = None
            ...
            with self.lock:
                if self.has_fts and keywords:
                    ...
                    for kw in keywords:
                        ...
                        params.append(f"%{kw}%")
                    sql = f"SELECT filename, full_path, size, mtime, is_dir FROM files WHERE {' AND '.join(wheres)}"
                
                sql += f" LIMIT {limit}"
                
                try:
                    raw_results = self.conn.execute(sql, params).fetchall()
                    ...
                    for kw in keywords:
                        wheres.append("filename_lower LIKE ?")
                        params.append(f"%{kw}%")
                    sql = f"SELECT filename, full_path, size, mtime, is_dir FROM files WHERE {' AND '.join(wheres)} LIMIT {limit}"
                    raw_results = self.conn.execute(sql, params).fetchall()
                ...
                for row in raw_results:
                    filename, full_path, size, mtime, is_dir = row
                    path_lower = full_path.lower()
                    
                    if scope_targets_lower:
                        in_scope = is_in_allowed_paths(path_lower, scope_targets_lower)
                        if not in_scope:
                            continue
    ...
    def build_index(self, drives, progress_cb=None, stop_fn=None):
        ...
        try:
            with self.lock:
                ...
                if self.has_fts:
                    try:
                        ...
                        logger.warning(f"清空FTS表失败: {e}")
                self.conn.commit()
            self.file_count = 0

            all_drives = []
            c_allowed_paths = None
            
            for d in drives:
                up = d.upper()
                drv_letter = up.rstrip('\\').rstrip(':')
                if drv_letter not in all_drives:
                    all_drives.append(drv_letter)
                if up.startswith('C:'):
                    c_allowed_paths = get_c_scan_dirs(self.config_mgr)

            c_dirs_display = [os.path.basename(p) for p in (c_allowed_paths or [])]
            logger.info(f"🔧 构建索引: 盘符 {all_drives}")
            if c_allowed_paths:
                logger.info(f"   C盘限制目录: {c_dirs_display}")

            # ========== 新增：优先尝试使用 C++ MFT 扫描器 ==========
            used_cpp = False

            if all_drives and IS_WINDOWS and getattr(self, "cpp_scanner", None) is not None:
                try:
                    drive_str = "".join([d[0] for d in all_drives if d])
                    logger.info(f"🚀 使用 C++ MFT 扫描器构建索引: 盘符 {drive_str}")

                    cpp_count = self.cpp_scanner.scan_drives(drive_str, enable_filter=False)
                    scan_ms = self.cpp_scanner.get_scan_time_ms()
                    logger.info(f"    [C++] 扫描完成: {cpp_count:,} 条记录, 耗时 {scan_ms:.2f} ms")

                    if cpp_count > 0:
                        records = self.cpp_scanner.get_all_records()
                        logger.info(f"    [C++] 获取记录: {len(records):,} 条，开始 Python 过滤 + 写入 DB")

                        self._consume_cpp_records(records, c_allowed_paths, progress_cb, stop_fn)
                        self.used_mft = True
                        MFT_AVAILABLE = True
                        used_cpp = True
                    else:
                        logger.warning("C++ 扫描返回 0 条，回退到 Python MFT")
                except Exception as e:
                    logger.error(f"❌ C++ MFT 扫描失败，将使用 Python MFT: {e}")
                    used_cpp = False

            # ========== 原来的 Python MFT 扫描逻辑，作为降级方案 ==========
            mft_scanned_drives = []
            if not used_cpp and all_drives and IS_WINDOWS:
                all_data = []
                lock = threading.Lock()
                ...
                def scan_one(drv):
                    try:
                        ...
                        if allowed:
                            logger.info(f"[MFT] 开始扫描 {drv}: (限制 {len(allowed)} 个目录)...")
                        else:
                            logger.info(f"[MFT] 开始扫描 {drv}: ...")
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(all_drives), 8)) as ex:
                    futures = {ex.submit(scan_one, d): d for d in all_drives}
                    for f in concurrent.futures.as_completed(futures):
                        ...
                        if progress_cb:
                            progress_cb(len(all_data), f"MFT {drv}:")
                ...
                if len(all_data) > 0:
                    ...
                    with self.lock:
                        ...
                        for i in range(0, len(all_data), 100000):
                            batch = all_data[i:i+100000]
                            cursor.executemany("INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)", batch)
                            logger.info(f"  [写入] {min(i+100000, len(all_data)):,}/{len(all_data):,}")
                ...
                for drv in failed:
                    if stop_fn and stop_fn():
                        break
                    if drv == 'C' and c_allowed_paths:
                        for target in c_allowed_paths:
                            logger.info(f"[传统扫描] C盘目录: {target}")
                            self._scan_dir(target, c_allowed_paths, progress_cb, stop_fn)
                    else:
                        self._scan_dir(f"{drv}:\\", None, progress_cb, stop_fn)

            # ========== MFT 补充验证（保持原逻辑） ==========
            if mft_scanned_drives and not (stop_fn and stop_fn()):
                logger.info("\n🔍 开始MFT补充验证...")
                supplement_count = self._verify_and_supplement(
                    mft_scanned_drives, 
                    c_allowed_paths,
                    progress_cb, 
                    stop_fn
                )
                logger.info(f"✅ 补充验证完成，新增 {supplement_count} 个文件")
            ...
            with self.lock:
                ...
                self.conn.commit()
                logger.info("📊 正在优化索引...")
                self.conn.execute("ANALYZE files")
                self.conn.commit()
            
            self._load_stats()
            elapsed = time.time() - build_start
            mft_str = "MFT✅" if self.used_mft else "传统扫描"
            logger.info(f"✅ 索引完成: {self.file_count:,} 条 ({mft_str}), 耗时 {elapsed:.2f}s")
            ...
            self.is_building = False
    def _consume_cpp_records(self, records, c_allowed_paths, progress_cb=None, stop_fn=None):
        """
        把 C++ MFT 扫描出来的 FileRecord 列表：
        - 调用 C++ build_path(ref) 构建完整路径
        - 使用 should_skip_path / should_skip_dir / SKIP_EXTS 过滤
        - os.stat 获取 size / mtime
        - 批量插入 SQLite
        """
        total = len(records)
        batch = []
        ...
        c_allowed_lower = None
        if c_allowed_paths:
            c_allowed_lower = [p.lower().rstrip('\\') for p in c_allowed_paths]
        ...
        for idx, rec in enumerate(records):
            ...
            full_path = self.cpp_scanner.build_path(rec.ref)
            if not full_path:
                continue
            path_lower = full_path.lower()

            if c_allowed_lower and path_lower.startswith('c:'):
                if not is_in_allowed_paths(path_lower, c_allowed_lower):
                    continue

            if should_skip_path(path_lower, c_allowed_lower):
                continue

            if is_dir:
                if should_skip_dir(name_lower, path_lower, c_allowed_lower):
                    continue
            else:
                ext = os.path.splitext(name_lower)[1]
                if ext in SKIP_EXTS:
            ...
            ext = "" if is_dir else os.path.splitext(name_lower)[1]
            ...
            if len(batch) >= 20000:
                ...
                inserted += len(batch)
                if progress_cb and (time.time() - last_progress_time) > 0.2:
                    progress_cb(self.file_count, f"C++ MFT: {idx}/{total}")
                    last_progress_time = time.time()
                batch = []
        ...
        logger.info(f"    [C++ MFT] Python 过滤后写入: {inserted:,} 条")

    def _verify_and_supplement(self, drives, c_allowed_paths=None, progress_cb=None, stop_fn=None):
        """验证MFT结果并补充遗漏的文件"""
        supplement_total = 0
        ...
        for drv in drives:
            ...
            logger.info(f"  [验证] {drv}: 盘...")
            
            with self.lock:
                indexed_dirs = set()
                rows = self.conn.execute(
                    "SELECT DISTINCT parent_dir FROM files WHERE full_path LIKE ?",
                    (f"{drv}:%",)
                ).fetchall()
                for (d,) in rows:
                    if drv == 'C' and c_allowed_lower:
                        d_lower = d.lower()
                        if is_in_allowed_paths(d_lower, c_allowed_lower):
                            indexed_dirs.add(d)
                    else:
                        indexed_dirs.add(d)
            ...
            for dir_path in indexed_dirs:
                ...
                try:
                    checked += 1
                    if checked % 500 == 0 and progress_cb:
                        progress_cb(self.file_count + supplement_count, f"验证 {drv}: {checked}/{total_dirs}")
                            ...
                            try:
                                ...
                                if not is_dir:
                                    ...
                                    if ext not in SKIP_EXTS:
                                        actual_files[entry.name] = (entry.path, st.st_size, st.st_mtime)
                    ...
                    with self.lock:
                        indexed = set()
                        rows = self.conn.execute(
                            "SELECT filename FROM files WHERE parent_dir = ? AND is_dir = 0",
                            (dir_path,)
                        ).fetchall()
                        for (name,) in rows:
                            indexed.add(name)
                    
                    for name, (full_path, size, mtime) in actual_files.items():
                        if name not in indexed:
                            ext = os.path.splitext(name)[1].lower()
                            batch.append((name, name.lower(), full_path, dir_path, ext, size, mtime, 0))
                            supplement_count += 1
            ...
            logger.info(f"    {drv}: 检查 {checked} 个目录，补充 {supplement_count} 个文件")
            supplement_total += supplement_count
        
        self.file_count += supplement_total
    ...
    def _scan_dir(self, target, allowed_paths=None, progress_cb=None, stop_fn=None):
        ...
        while stack:
            ...
            if should_skip_path(cur_lower, allowed_paths_lower):
                continue
    ...
    def rebuild_drive(self, drive_letter, progress_cb=None, stop_fn=None):
        ...
        try:
            ...
            with self.lock:
                ...
                self.conn.execute("DELETE FROM files WHERE full_path LIKE ?", (drive_pattern,))
                self.conn.commit()
            
            # 获取扫描目录
            if drive == 'C':
                scan_paths = get_c_scan_dirs(self.config_mgr)
            else:
                scan_paths = [f"{drive}:\\"]
            ...
            # 尝试MFT扫描
            if IS_WINDOWS and drive != 'C':
                try:
                    allowed = None
                    logger.info(f"[MFT] 开始扫描 {drive}: ...")
                    data = enum_volume_files_mft(drive, SKIP_DIRS_LOWER, SKIP_EXTS, allowed_paths=allowed)
                    
                    if data:
                        logger.info(f"[MFT] {drive}: 写入 {len(data):,} 条记录")
                        with self.lock:
                            self.conn.executemany(
                                "INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)",
                                data
                ...
                # C盘或非Windows使用传统扫描
                c_allowed = scan_paths if drive == 'C' else None
                
                if drive == 'C':
                    # C盘尝试MFT
                    try:
                        logger.info(f"[MFT] 开始扫描 C: (限制 {len(scan_paths)} 个目录)...")
                        data = enum_volume_files_mft('C', SKIP_DIRS_LOWER, SKIP_EXTS, allowed_paths=scan_paths)
                        
                        if data:
                            logger.info(f"[MFT] C: 写入 {len(data):,} 条记录")
                            with self.lock:
                                self.conn.executemany(
                                    "INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)",
                                    data
                                ...
                                self.conn.commit()
                            if progress_cb:
                                progress_cb(len(data), "MFT C:")
                    except Exception as e:
                        logger.warning(f"[MFT] C: 失败，使用传统扫描 - {e}")
                        for path in scan_paths:
                            if stop_fn and stop_fn():
                                break
                            logger.info(f"[传统扫描] {path}")
                            self._scan_dir(path, scan_paths, progress_cb, stop_fn)
                else:
                    for path in scan_paths:
                        if stop_fn and stop_fn():
                            break
        ...
        try:
            for _ in range(200):
                ...
                if t == "BATCH":
                    ...
                    # 更新搜索进度
                    scanned_dirs, current_path = d
                    elapsed = time.time() - self.search_stats['start_time']
                    speed = scanned_dirs / elapsed if elapsed > 0 else 0
                    self.status_path.set(f"扫描: {scanned_dirs} 目录 ({speed:.0f}/s) - {current_path[-50:]}")
                elif t == "IDX_PROG":
                    self._check_index()
