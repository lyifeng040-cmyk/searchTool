    ...
    def search(self, keywords, scope_targets, limit=50000):
        ...
        try:
            with self.lock:
                ...
                # ★ 优先使用 FTS5（速度快 10 倍以上）
                if self.has_fts and len(keywords) == 1 and not any(c in keywords[0] for c in '*?"'):
                    try:
                        # FTS5 查询
                        fts_kw = keywords[0] + "*"  # 前缀匹配
                        sql = """
                            ...
                            LIMIT ?
                        """
                        raw_results = list(cursor.execute(sql, (fts_kw, limit)))
                    except Exception as e:
                        logger.debug(f"FTS5 查询失败，回退 LIKE: {e}")
                        raw_results = self._search_like(cursor, keywords, limit)
                else:
                    # 多关键词或特殊字符，用 LIKE
                    raw_results = self._search_like(cursor, keywords, limit)

                # scope 标准化：拆成 drives + paths
                scope_drives = set()
                scope_paths = []
    ...
    def _render_page(self):
        ...
        try:
            for i, item in enumerate(page_items):
                ...
                if i % 2 == 1:
                    for col in range(4):
                        q_item.setBackground(col, QColor("#f8f9fa"))
