...
HAS_RUST_ENGINE_V2 = False
RUST_ENGINE = None

if IS_WINDOWS:
    try:
        # V2 API 结果结构
        class ScanResultV2(ctypes.Structure):
            _fields_ = [
        ...
        # V1 API 结果结构（兼容旧版）
        class ScanResultPacked(ctypes.Structure):
            _fields_ = [
        ...
        if dll_path:
            ...
            # ===== V2 API（新版，全功能）=====
            # scan_and_index
            RUST_ENGINE.scan_and_index.argtypes = [
                ctypes.c_char_p,  # db_path
                ctypes.c_char_p,  # drives
                ctypes.c_char_p,  # c_allowed_paths
                ctypes.c_void_p,  # progress_callback
            ]
            RUST_ENGINE.scan_and_index.restype = ScanResultV2
            
            # rebuild_drive_index
            RUST_ENGINE.rebuild_drive_index.argtypes = [
                ctypes.c_char_p,  # db_path
                ctypes.c_char,    # drive_letter
                ctypes.c_char_p,  # c_allowed_paths
                ctypes.c_void_p,  # progress_callback
            ]
            RUST_ENGINE.rebuild_drive_index.restype = ScanResultV2
            
            # free_error_msg
            RUST_ENGINE.free_error_msg.argtypes = [ctypes.c_char_p]
            RUST_ENGINE.free_error_msg.restype = None
            
            # get_version
            RUST_ENGINE.get_version.argtypes = []
            RUST_ENGINE.get_version.restype = ctypes.c_char_p
            
            # ===== V1 API（兼容旧版）=====
            RUST_ENGINE.scan_drive_packed.argtypes = [ctypes.c_uint16]
            RUST_ENGINE.scan_drive_packed.restype = ScanResultPacked
            
            RUST_ENGINE.free_scan_result.argtypes = [ScanResultPacked]
            RUST_ENGINE.free_scan_result.restype = None

            # 检查版本
            try:
                version = RUST_ENGINE.get_version().decode('utf-8')
                if version.startswith("2."):
                    HAS_RUST_ENGINE_V2 = True
                    logger.info(f"✅ Rust 引擎 V2 加载成功: {dll_path} (版本 {version})")
                else:
                    logger.info(f"✅ Rust 引擎 V1 加载成功: {dll_path}")
            except Exception:
                pass
...
if IS_WINDOWS:
    ...
    # ==================== 新的优化版函数（替换这里）====================
    def _batch_stat_files(py_list):
        """批量获取文件大小和修改时间 - 高性能优化版"""
        files_to_stat = [item for item in py_list if item[7] == 0]  # 只处理文件，不处理目录
        if not files_to_stat:
            return

        total_files = len(files_to_stat)
        logger.info(f"获取 {total_files} 个文件的大小...")
        start_time = time.time()

        # 使用 Windows API 直接获取文件属性，比 os.stat 更快
        ...
        # FILETIME 转 Unix 时间戳的常量
        EPOCH_DIFF = 116444736000000000

        def stat_worker(items_batch):
            """工作线程：批量获取文件属性"""
                ...
                try:
                    ...
                    if GetFileAttributesExW(path, 0, ctypes.byref(data)):
                        ...
                        if mtime_ft > EPOCH_DIFF:
                            item[6] = (mtime_ft - EPOCH_DIFF) / 10000000.0
                        else:
                            item[6] = 0
                except Exception:
                    pass  # 忽略无法访问的文件
        ...
        # 计算每个线程处理的批次大小
        batch_size = max(100, (total_files + num_workers - 1) // num_workers)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i in range(0, total_files, batch_size):
                ...
                futures.append(executor.submit(stat_worker, batch))
        ...
        logger.info(f"文件大小获取完成: {total_files} 个文件, 耗时 {elapsed:.2f}s, 速度 {speed:.0f}/s")
...
class IndexManager(QObject):
    ...
    def _reopen_connection(self):
        ...
        try:
            with self.lock:
                ...
                logger.info("数据库连接已重新打开")
        except Exception as e:
            logger.error(f"重新打开数据库连接失败: {e}")
            # 尝试重新初始化
            try:
                self._init_db()
            except:
    ...
    def search(self, keywords, scope_targets, limit=50000):
        ...
        try:
            with self.lock:
                ...
                filtered = []
                scope_targets_lower = (
                    [os.path.normpath(t).lower() for t in scope_targets]
                    if scope_targets
                    else None
                )
                ...
                for row in raw_results:
                    ...
                    if scope_targets_lower:
                        ...
                        if path_drive in scope_targets_lower:
                            pass
                        elif any(
                            path_lower.startswith(allowed)
                            for allowed in scope_targets_lower
                        ):
                            pass
                        else:
                            continue
    ...
    def build_index(self, drives, stop_fn=None):
        ...
        try:
            if not HAS_RUST_ENGINE_V2:
                logger.error("Rust V2 引擎不可用，无法构建索引（专用 V2 模式）")
                return

            logger.info("🚀 使用 Rust V2 引擎构建索引...")
            self.progress_signal.emit(0, "Rust V2: 扫描文件系统...")

            # 1. 关闭 Python 这边的 SQLite 连接，避免 database is locked
            with self.lock:
                if self.conn:
                    try:
                        self.conn.close()
                        ...
                        pass
                    self.conn = None
                    logger.info("临时关闭数据库连接")

            # 2. 准备参数
            db_path_bytes = self.db_path.encode("utf-8")
            drives_str = ",".join(
                [d.rstrip(":\\").upper() for d in drives if os.path.exists(d)]
            ).encode("utf-8")
            c_paths_str = ",".join(get_c_scan_dirs(self.config_mgr)).encode("utf-8")

            # 3. 调用 Rust 核心
            result = RUST_ENGINE.scan_and_index(
                db_path_bytes,
                drives_str,
                c_paths_str,
                None,  # 暂不传进度回调，有需要可以后面加
            )

            if not result.success:
                err = result.error_msg.decode("utf-8") if result.error_msg else "未知错误"
                logger.error(f"❌ Rust V2 引擎失败: {err}")
                if result.error_msg:
                    RUST_ENGINE.free_error_msg(result.error_msg)
                return  # 专用 V2 模式，不回退

            # 4. Rust 成功，重新打开连接并加载统计
            self._reopen_connection()
            self.force_reload_stats()
            self.is_ready = True
            MFT_AVAILABLE = True

            elapsed = time.time() - build_start
            logger.info(
                f"✅ Rust V2 索引构建完成: {self.file_count:,} 条, 耗时 {elapsed:.2f}s"
            )
            self.progress_signal.emit(self.file_count, f"完成: {self.file_count:,} 条")

        except Exception as e:
            logger.error(f"❌ Rust V2 构建异常: {e}")
            import traceback
            traceback.print_exc()
        finally:
            self.is_building = False
        ...
        # ★★★ 回退到旧方法 ★★★
        self._build_index_fallback(drives, stop_fn)
    ...
    def _reopen_connection(self):
        ...
        try:
            ...
            with self.lock:
                ...
                cursor.execute("PRAGMA synchronous=NORMAL")
                cursor.execute("PRAGMA journal_mode=WAL")
                cursor.execute("INSERT OR REPLACE INTO meta (key, value) VALUES ('build_time', ?)", (str(time.time()),))
                cursor.execute("INSERT OR REPLACE INTO meta (key, value) VALUES ('used_mft', ?)", ("1" if self.used_mft else "0",))
                if not HAS_APSW:
                    self.conn.commit()
            ...
            def build_fts_async():
                try:
                    ...
                    with self.lock:
                        cursor = self.conn.cursor()
                        cursor.execute("CREATE VIRTUAL TABLE IF NOT EXISTS files_fts USING fts5(filename, content=files, content_rowid=id)")
            ...
            logger.info(f"✅ 索引构建完成: {self.file_count:,} 条, 耗时 {time.time() - build_start:.2f}s")
            self.is_ready = self.file_count > 0
            self.build_finished_signal.emit()
    ...
    def rebuild_drive(self, drive_letter, progress_callback=None, stop_fn=None):
        """重建单个驱动器索引（专用 V2 版，不回退）"""
        if self.is_building:
            logger.warning("索引正在构建中，跳过")
            return

        if not HAS_RUST_ENGINE_V2:
            logger.error("Rust V2 引擎不可用，无法重建驱动器索引（专用 V2 模式）")
            return
        ...
        try:
            logger.info(f"🚀 使用 Rust V2 重建 {drive}: 盘索引...")
            ...
            with self.lock:
                if self.conn:
                    try:
                        ...
                        pass
                    self.conn = None
                    logger.info("临时关闭数据库连接（重建单盘）")

            db_path_bytes = self.db_path.encode("utf-8")
            c_paths_str = ",".join(get_c_scan_dirs(self.config_mgr)).encode("utf-8")
            drive_char = drive.encode("utf-8")[0]

            result = RUST_ENGINE.rebuild_drive_index(
                db_path_bytes,
                drive_char,
                c_paths_str,
                None,
            ...
            if not result.success:
                err = result.error_msg.decode("utf-8") if result.error_msg else "未知错误"
                logger.error(f"❌ Rust V2 重建 {drive} 失败: {err}")
                if result.error_msg:
                    RUST_ENGINE.free_error_msg(result.error_msg)
                return  # 专用 V2 模式，不回退
            ...
            elapsed = time.time() - start_time
            logger.info(
                f"✅ {drive}: 盘索引重建完成（Rust V2），当前总数 {self.file_count:,}，耗时 {elapsed:.2f}s"
            )
    ...
    def _rebuild_drive_fallback(self, drive, progress_callback=None, stop_fn=None):
        """回退到旧的重建方法"""
        logger.info(f"使用 Python 方法重建 {drive}: 盘索引...")
...
class SearchApp(QMainWindow):
    ...
    def _update_page_info(self):
        ...
        self.lbl_page.setText(
            f"第 {self.current_page}/{self.total_pages} 页 ({total}项)"
        )
    ...
    def sort_column(self, logical_index):
        ...
        with self.results_lock:
            ...
            if logical_index == 0:  # 文件名
                self.filtered_results.sort(
                    key=lambda x: x["filename"].lower(), reverse=reverse
                )
            elif logical_index == 1:  # 目录
                self.filtered_results.sort(
                    key=lambda x: x["dir_path"].lower(), reverse=reverse
                )
            elif logical_index == 2:  # 大小
                self.filtered_results.sort(key=lambda x: x["size"], reverse=reverse)
            elif logical_index == 3:  # 修改时间
                self.filtered_results.sort(key=lambda x: x["mtime"], reverse=reverse)
