...
class IndexManager(QObject):
    ...
    def _parse_search_syntax(self, keyword_str):
        """
        解析搜索语法，返回 (keywords, filters)
        
        支持语法：
        - ext:pdf → 扩展名过滤
        - size:>10mb / size:<1kb → 大小过滤
        - dm:today / dm:7d / dm:30d → 修改时间过滤
        - folder: → 只搜目录
        - file: → 只搜文件
        - path:xxx → 路径包含
        """
        ...
        filters = {
            'ext': None,        # 扩展名
            'size_min': 0,      # 最小大小
            'size_max': 0,      # 最大大小（0表示不限）
            'dm_after': 0,      # 修改时间晚于（时间戳）
            'type': None,       # 'folder' / 'file' / None
            'path': None,       # 路径包含
        }
