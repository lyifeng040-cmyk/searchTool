...
if IS_WINDOWS:
    ...
    def _batch_stat_files(py_list):
        ...
        num_dirs = len(dir_files)

        # Windows API 定义（只定义一次，避免重复）
        ...
        def process_directory(dir_path, file_items):
            """处理单个目录 - 使用 FindFirstFile"""
            if not file_items:
                return
            
            name_to_items = defaultdict(list)
            for item in file_items:
                key = item[1] if item[1] else item[0].lower()
                name_to_items[key].append(item)
            
            processed_in_dir = set()
            find_data = WIN32_FIND_DATAW()
            
            try:
                search_path = dir_path.rstrip('\\') + "\\*"
                    ...
                    try:
                        while True:
                            ...
                            if fname and fname not in ('.', '..'):
                                ...
                                if fname_lower in name_to_items:
                                    size = (find_data.nFileSizeHigh << 32) | find_data.nFileSizeLow
                                    ft = (find_data.ftLastWriteTime.dwHighDateTime << 32) | find_data.ftLastWriteTime.dwLowDateTime
                                    mtime = (ft - EPOCH_DIFF) / TICKS_PER_SEC if ft > EPOCH_DIFF else 0
                                    
                                    for item in name_to_items[fname_lower]:
                                        item[5] = size
                                        item[6] = mtime
                                    
                                    processed_in_dir.add(fname_lower)
                                    with lock:
                                        success_count[0] += len(name_to_items[fname_lower])
            ...
            # 收集未处理的文件
            for fname_lower, items in name_to_items.items():
                if fname_lower not in processed_in_dir:
                    with lock:
                        pending_items.extend(items)

        def process_pending_batch(items_batch):
            """处理未成功的文件 - 使用 GetFileAttributesExW（比os.stat快）"""
            ...
            for item in items_batch:
                try:
                    if GetFileAttributesExW(item[2], 0, ctypes.byref(data)):
                        ...
                        ft = (data.ftLastWriteTime.dwHighDateTime << 32) | data.ftLastWriteTime.dwLowDateTime
                        item[6] = (ft - EPOCH_DIFF) / TICKS_PER_SEC if ft > EPOCH_DIFF else 0
                        count += 1
                except:
                    pass
            return count
        ...
        logger.info(f"阶段1: 使用 {num_workers} 线程批量处理 {num_dirs} 个目录")

        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = [
                executor.submit(process_directory, dir_path, items)
                for dir_path, items in dir_files.items()
            ]
            concurrent.futures.wait(futures, timeout=120)

        phase1_time = time.time() - start_time
        phase1_count = success_count[0]
        logger.info(f"阶段1完成: {phase1_count} 个文件, 耗时 {phase1_time:.2f}s")

        # 阶段2：处理未成功的文件（使用更快的 API）
        if pending_items:
            pending_count = len(pending_items)
            logger.info(f"阶段2: 处理 {pending_count} 个未获取的文件")
            
            phase2_start = time.time()
            
            # 使用更多线程并行处理
            num_workers_2 = min(64, max(16, pending_count // 500))
            batch_size = max(50, (pending_count + num_workers_2 - 1) // num_workers_2)
            
            with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers_2) as executor:
                futures = []
                for i in range(0, pending_count, batch_size):
                    batch = pending_items[i:i + batch_size]
                    futures.append(executor.submit(process_pending_batch, batch))
                
                results = [f.result() for f in concurrent.futures.as_completed(futures)]
                phase2_success = sum(results)
                success_count[0] += phase2_success
            
            phase2_time = time.time() - phase2_start
            logger.info(f"阶段2完成: {phase2_success}/{pending_count} 个文件, 耗时 {phase2_time:.2f}s")

        elapsed = time.time() - start_time
        speed = success_count[0] / elapsed if elapsed > 0 else 0
        logger.info(f"文件大小获取完成: {success_count[0]}/{total_files} 个文件, "
                    f"耗时 {elapsed:.2f}s, 速度 {speed:.0f}/s")
    ...
    def build_index(self, drives, stop_fn=None):
        ...
        try:
            ...
            if all_data:
                ...
                for i in range(0, len(all_data), batch_size):
                    ...
                    try:
                        ...
                        logger.error(f"写入批次失败: {e}")

                self.file_count = total_written
                logger.info(
                    f"✅ 阶段3完成: {time.time() - build_start:.2f}s, 写入 {total_written:,} 条"
                )
            ...
            try:
                with self.lock:
                    cursor = self.conn.cursor()
                    final_count = list(cursor.execute("SELECT COUNT(*) FROM files"))[0][
                        0
                    ]
            ...
            logger.info(
                f"✅ 索引构建完成: {self.file_count:,} 条, 耗时 {time.time() - build_start:.2f}s"
            )
            self.is_ready = self.file_count > 0
    ...
    def rebuild_drive(self, drive_letter, progress_callback=None, stop_fn=None):
        ...
        try:
            ...
            with self.lock:
                cursor = self.conn.cursor()
                cursor.execute(
                    "DELETE FROM files WHERE full_path LIKE ?", (f"{drive}:%",)
                )
                if not HAS_APSW:
                    self.conn.commit()
            ...
            try:
                data = enum_volume_files_mft(
                    drive, SKIP_DIRS_LOWER, SKIP_EXTS, allowed_paths
                )
                ...
                with self.lock:
                    cursor = self.conn.cursor()
                    cursor.executemany(
                        "INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)", data
                    )
                    cursor.execute(
                        "INSERT OR REPLACE INTO meta (key, value) VALUES ('build_time', ?)",
                        (str(time.time()),),
                ...
                if progress_callback:
                    progress_callback(len(data), f"完成 {drive}:")

            except Exception as e:
                logger.error(f"MFT扫描失败，使用传统扫描: {e}")
                paths_to_scan = c_allowed_paths if drive == "C" else [f"{drive}:\\"]
                for path in paths_to_scan:
                    self._scan_dir(
                        path, c_allowed_paths if drive == "C" else None, stop_fn
                    )
