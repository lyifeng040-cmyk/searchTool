...
def create_worker(index_mgr, kw, scope_targets, regex_var, force_realtime, main_window=None):
    ...
    if use_idx:
        worker = IndexSearchWorker(index_mgr, kw, scope_targets, regex_var)
        return worker, False
    else:
        worker = RealtimeSearchWorker(kw, scope_targets, regex_var)
        return worker, True
