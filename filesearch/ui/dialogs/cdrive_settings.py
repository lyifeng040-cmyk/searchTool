"""
C: drive settings dialog extracted from legacy implementation.
"""
...
class CDriveSettingsDialog:
	...
	def _manual_add(self):
		text, ok = QInputDialog.getText(self.dialog, "手动输入C盘目录路径", "路径:", QLineEdit.Normal, "")
		if ok and text:
	...
	def _delete_path(self, path):
		if QMessageBox.question(self.dialog, "确认", f"确定删除此目录？\n{path}") != QMessageBox.Yes:
			return
	...
	def _reset_default(self):
		if QMessageBox.question(self.dialog, "确认", "确定恢复系统默认目录？\n这将清空当前列表。") == QMessageBox.Yes:
	...
	def _save(self):
		...
		if has_changes:
			...
			if result == QMessageBox.Yes:
				...
				QMessageBox.information(self.dialog, "提示", "设置已保存，稍后可手动重建C盘索引")
				self.dialog.accept()
		else:
	...
	def _rebuild_c_drive(self):
		...
		if QMessageBox.question(self.dialog, "确认", "确定立即重建C盘索引?") == QMessageBox.Yes:
			self.dialog.accept()
			self._do_rebuild_c_drive()
