...
class SearchApp(QMainWindow):
    def __init__(self, db_path=None):
        ...
        # ========= 原脚本中 style/theme 的位置，现在用简单主题名保存 =========
        saved_theme = self.config_mgr.get_theme()  # 原来是 ttk.Style(saved_theme)
        ...
        self.force_realtime = False      # 原来是 tk.BooleanVar
        self.fuzzy_var = True            # 原来是 tk.BooleanVar
        self.regex_var = False           # 原来是 tk.BooleanVar
        ...
        self.mini_search = MiniSearchWindow(self)  # 注意：这里的 MiniSearchWindow 之前已经按原逻辑转换为 PySide6
  ...
  Ctrl+Shift+Tab    主窗口
    ...
    def _render_page(self):
        ...
        for i, item in enumerate(page_items):
            ...
            q_item = QTreeWidgetItem(row)
            # 行背景色（odd/even）
            if i % 2 == 0:
                q_item.setBackground(0, QColor("#ffffff"))
                q_item.setBackground(1, QColor("#ffffff"))
                q_item.setBackground(2, QColor("#ffffff"))
                q_item.setBackground(3, QColor("#ffffff"))
            else:
                q_item.setBackground(0, QColor("#f8f9fa"))
                q_item.setBackground(1, QColor("#f8f9fa"))
                q_item.setBackground(2, QColor("#f8f9fa"))
                q_item.setBackground(3, QColor("#f8f9fa"))
    ...
    def stop_search(self):
        ...
        self.status.setText(f"🛑 已停止 ({count}项)")
    ...
    def _search_idx(self, sid, keywords, scope_targets):
        try:
            ...
            if results is None:
                self.result_queue.put(("ERROR", "索引不可用"))
                return
        ...
        try:
            for _ in range(200):
                ...
                if t == "BATCH":
                    ...
                    self._reset_ui()
                    self.status.setText(f"✅ 完成: {self.total_found}项 ({d:.2f}s)")
                    self._finalize()
                elif t == "ERROR":
                    self._reset_ui()
                    QMessageBox.warning(self, "错误", d)
                elif t == "PROGRESS":
                    scanned_dirs, _ = d
                    ...
                    self._check_index()
                    self.status_path.setText(f"索引: {d[1][-40:]}")
                elif t == "IDX_DONE":
                    self._check_index()
            ...
            if self.is_searching:
                ...
                if result_count > 0:
                    ...
                    if (
                        ...
                        self.status.setText(f"已找到: {result_count}")
        except Exception as e:
