...
if IS_WINDOWS:
    ...
    def enum_volume_files_mft(drive_letter, skip_dirs, skip_exts, allowed_paths=None):
        ...
        if HAS_RUST_ENGINE:
            ...
            try:
                # C盘传入白名单给Rust预过滤
                if drive_letter.upper() == 'C' and allowed_paths:
                    allowed_str = '\0'.join(p.lower().rstrip('\\') for p in allowed_paths)
                    allowed_bytes = allowed_str.encode('utf-8')
                    result = RUST_ENGINE.scan_drive_packed(
                        ord(drive_letter.upper()[0]),
                        allowed_bytes,
                        len(allowed_bytes)
                    )
                else:
                ...
                while off < n:
                    # 格式：24字节头 + name + parent + ext
                    # [is_dir:1][name_len:2][parent_len:2][path_len:2][ext_len:1][size:8][mtime:8]
                    if off + 24 > n:
                        break

                    is_dir = raw_data[off] == 1
                    name_len = int.from_bytes(raw_data[off + 1:off + 3], "little")
                    parent_len = int.from_bytes(raw_data[off + 3:off + 5], "little")
                    path_len = int.from_bytes(raw_data[off + 5:off + 7], "little")  # 仅供参考
                    ext_len = raw_data[off + 7]
                    size = int.from_bytes(raw_data[off + 8:off + 16], "little")
                    mtime = struct.unpack("<d", raw_data[off + 16:off + 24])[0]
                    off += 24

                    # 数据部分：name + parent + ext（没有单独的path字段）
                    total_len = name_len + parent_len + ext_len
                    if off + total_len > n:
                        break
                ...
                logger.info(f"✅ Rust返回={result.count}, 解析={len(py_list)}")
