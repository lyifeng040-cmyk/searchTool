    ...
    def enum_volume_files_mft(drive_letter, skip_dirs, skip_exts, allowed_paths=None):
        ...
        if HAS_RUST_ENGINE:
            ...
            try:
                ...
                while off < n:
                    # ★ 新格式解析（包含 size 和 mtime）
                    # [is_dir:1][name_len:2][name_lower_len:2][path_len:2][parent_len:2][ext_len:1][size:8][mtime:8][data...]
                    if off + 26 > n:  # 最小头部长度
                        break
                    ...
                    size = int.from_bytes(raw_data[off + 10:off + 18], "little")
                    mtime = struct.unpack('<d', raw_data[off + 18:off + 26])[0]  # f64
                    off += 26
...
class UsnFileWatcher(QObject):
    ...
    # ★ 添加信号
    files_changed = Signal(int, int)  # (新增数量, 删除数量)
                ...
                for i in range(result.count):
                    ...
                    if c.path_ptr and c.path_len > 0:
                        try:
                            ...
                            logger.debug(f"[USN监控] 解析路径失败: {e}")

                if changes:
                    logger.debug(f"[USN监控] {drive}: 检测到 {len(changes)} 个变更")
                    self._apply_changes(changes)

                RUST_ENGINE.free_change_list(result)

            new_usn = RUST_ENGINE.get_current_usn(ord(drive))
            if new_usn > last_usn:
                self.usn_positions[drive] = new_usn

        except Exception as e:
            logger.error(f"[USN监控] 检查 {drive} 失败: {e}")
    ...
    def _apply_changes(self, changes):
                    ...
                    if os.path.exists(path):
                        if is_dir:
                            if not should_skip_dir(name.lower(), path.lower()):
                                inserts.append((
                                    name, name.lower(), path, os.path.dirname(path),
                                    "", 0, 0, 1
                                ))
                        else:
                            ext = os.path.splitext(name)[1].lower()
        ...
        try:
            ...
            logger.info(f"[USN监控] 更新: +{len(inserts)} -{len(deletes)}")
            
            # ★ 发送信号通知 UI
            self.files_changed.emit(len(inserts), len(deletes))
