...
class IndexManager:
        ...
        try:
            ...
            try:
                ...
                if not fts_exists:
                    ...
                    cursor.execute(
                        "CREATE TRIGGER IF NOT EXISTS files_ai AFTER INSERT ON files BEGIN INSERT INTO files_fts(rowid, filename) VALUES (new.id, new.filename); END"
                    )
                    cursor.execute(
                        "CREATE TRIGGER IF NOT EXISTS files_ad AFTER DELETE ON files BEGIN INSERT INTO files_fts(files_fts, rowid, filename) VALUES('delete', old.id, old.filename); END"
                    )
                self.has_fts = True
        ...
        try:
            with self.lock:
                ...
                if time_row and time_row[0]:
                    try:
                        self.last_build_time = float(time_row[0])
                ...
                if not preserve_mft:
                    mft_row = cursor.execute(
                        "SELECT value FROM meta WHERE key='used_mft'"
                    ).fetchone()
            ...
            for ext in ["", "-wal", "-shm"]:
                ...
                if os.path.exists(src):
                    try:
                        ...
                        logger.error(f"复制数据库文件失败 {src}: {e}")
    ...
    def search(self, keywords, scope_targets, limit=50000):
        ...
        try:
            with self.lock:
                ...
                filtered = []
                scope_targets_lower = (
                    [t.lower().rstrip("\\") for t in scope_targets]
                    if scope_targets
                    else None
                )
                ...
                for row in raw_results:
                    ...
                    if scope_targets_lower and not is_in_allowed_paths(
                        path_lower, scope_targets_lower
                    ):
                        continue
    ...
    def build_index(self, drives, progress_cb=None, stop_fn=None):
        ...
        try:
            ...
            logger.info(f"✅ 阶段1完成: {time.time() - phase_start:.2f}s")

            # ========== 阶段2: MFT扫描 ==========
            phase_start = time.time()
            if progress_cb:
                progress_cb(0, "阶段2/5: MFT扫描...")

            all_drives = [d.upper().rstrip(":\\") for d in drives if os.path.exists(d)]
            c_allowed_paths = (
                get_c_scan_dirs(self.config_mgr) if "C" in all_drives else None
            )

            logger.info(f"🔧 扫描盘符: {all_drives}")
            if c_allowed_paths:
                logger.info(
                    f"   C盘限制目录: {[os.path.basename(p) for p in c_allowed_paths]}"
                )
            ...
            if all_drives and IS_WINDOWS:
                ...
                def scan_one(drv):
                    try:
                        ...
                        logger.info(f"[MFT] 扫描 {drv}: (Rust: {HAS_RUST_ENGINE})")
            ...
            logger.info(
                f"✅ 阶段2完成: {time.time() - phase_start:.2f}s, 扫描到 {len(all_data):,} 条"
            )
            ...
            if all_data:
                phase_start = time.time()
                if progress_cb:
                    progress_cb(len(all_data), "阶段3/5: 写入数据库...")
                ...
                for i in range(0, len(all_data), batch_size):
                    ...
                    while retry_count < max_retries:
                        try:
                            ...
                            if retry_count < max_retries:
                                logger.warning(
                                    f"数据库忙，重试 {retry_count}/{max_retries}"
                                )
                                time.sleep(0.5)
                            else:
                                logger.error("写入失败，跳过此批次")
                        except Exception as e:
                    ...
                    if progress_cb:
                        progress_cb(
                            total_written, f"写入: {total_written:,}/{len(all_data):,}"
                        )

                self.file_count = total_written
                logger.info(
                    f"✅ 阶段3完成: {time.time() - phase_start:.2f}s, 写入 {total_written:,} 条"
                )

            # ========== 阶段4: 创建索引 ==========
            phase_start = time.time()
            if progress_cb:
                progress_cb(self.file_count, "阶段4/5: 创建索引...")
            ...
            logger.info(f"✅ 阶段4完成: {time.time() - phase_start:.2f}s")

            # ========== 阶段5: 后台构建FTS5 ==========
            if progress_cb:
                progress_cb(self.file_count, "阶段5/5: 构建全文索引(后台)...")
            ...
            def build_fts_async():
                ...
                try:
                    with self.lock:
                        cursor = self.conn.cursor()
                        cursor.execute(
                            "INSERT OR REPLACE INTO meta VALUES('build_time', ?)",
                            (str(time.time()),),
                        )
                        cursor.execute(
                            "INSERT OR REPLACE INTO meta VALUES('used_mft', ?)",
            ...
            for drv in failed_drives:
                ...
                for path in paths_to_scan:
                    ...
                    self._scan_dir(
                        path,
                        c_allowed_paths if drv == "C" else None,
                        progress_cb,
                        stop_fn,
                    )
            ...
            logger.info(
                f"✅ 索引构建完成: {self.file_count:,} 条, 耗时 {total_time:.2f}s"
            )
    ...
    def rebuild_drive(self, drive_letter, progress_cb=None, stop_fn=None):
        ...
        try:
            with self.lock:
                ...
                cursor.execute(
                    "DELETE FROM files WHERE full_path LIKE ?", (drive_pattern,)
                )

            scan_paths = (
                get_c_scan_dirs(self.config_mgr) if drive == "C" else [f"{drive}:\\"]
            )
            ...
            if IS_WINDOWS:
                try:
                    ...
                    for path in scan_paths:
                        self._scan_dir(
                            path,
                            (scan_paths if drive == "C" else None),
                            progress_cb,
                            stop_fn,
                        )
            else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error("监控数据库更新失败: 数据库持续繁忙")
            except apsw.Error as e:
...
class TrayManager:
    ...
    def start(self):
        ...
        try:
            ...
            self.icon = pystray.Icon("FileSearch", image, "极速文件搜索", menu)
...
class HotkeyManager:
    ...
    def _show_main_window(self):
        try:
            ...
            self.app.root.after(
                100, lambda: self.app.root.attributes("-topmost", False)
            )
            self.app.root.focus_force()
            self.app.entry_kw.focus()
...
class MiniSearchWindow:
    ...
    def _create_window(self):
        ...
        self.search_icon.bind(
            "<Enter>", lambda e: self.search_icon.configure(fg="#0088cc")
        )
        self.search_icon.bind(
            "<Leave>", lambda e: self.search_icon.configure(fg="#004466")
        )
        ...
        self.left_arrow.bind(
            "<Enter>", lambda e: self.left_arrow.configure(fg="#0088cc")
        )
        self.left_arrow.bind(
            "<Leave>", lambda e: self.left_arrow.configure(fg="#004466")
        )
        ...
        self.right_arrow.bind(
            "<Enter>", lambda e: self.right_arrow.configure(fg="#0088cc")
        )
        self.right_arrow.bind(
            "<Leave>", lambda e: self.right_arrow.configure(fg="#004466")
        )
    ...
    def _display_results(self, results):
        ...
        self.tip_label.config(
            text=f"找到 {len(self.results)} 个  │  Enter=打开  Ctrl+Enter=定位  Delete=删除  Tab=主页面  Esc=关闭"
        )
...
class CDriveSettingsDialog:
    ...
    def show(self):
        ...
        self.dialog.geometry(
            f"+{self.parent.winfo_x() + 100}+{self.parent.winfo_y() + 50}"
        )
    ...
    def _build_ui(self):
        ...
        ttk.Label(btn_row, text="扫描目录列表:", font=("微软雅黑", 10, "bold")).pack(
            side=LEFT
        )
        ...
        self.stat_label = ttk.Label(
            quick_row, text="", font=("微软雅黑", 9), foreground="#666"
        )
        ...
        ttk.Button(
            bottom_frame, text="保存", command=self._save, bootstyle="success", width=10
        ).pack(side=RIGHT, padx=(5, 0))
    ...
    def _refresh_paths_list(self):
        # 清理旧的 tooltip
        if hasattr(self, "_active_tooltips"):
        ...
        for i, item in enumerate(paths):
            ...
            cb = ttk.Checkbutton(
                row, variable=var, bootstyle="round-toggle", command=self._update_stats
            )
            cb.pack(side=LEFT)
            ...
            # tooltip 相关闭包
            def show_tooltip(event, full_path=path, label=lbl):
                if hasattr(label, "_tooltip") and label._tooltip:
                    try:
            ...
            def hide_tooltip(event, label=lbl):
                if hasattr(label, "_tooltip") and label._tooltip:
                    try:
                        label._tooltip.destroy()
                    except:
            ...
            lbl.bind("<Enter>", lambda e, fp=path, lb=lbl: show_tooltip(e, fp, lb))
            lbl.bind("<Leave>", lambda e, lb=lbl: hide_tooltip(e, lb))
    ...
    def _browse_add(self):
        path = filedialog.askdirectory(title="选择C盘目录", initialdir="C:\\")
        if path:
            self._add_path(path)
    ...
    def _manual_add(self):
        ...
        entry_menu.add_command(
            label="粘贴", command=lambda: entry.event_generate("<<Paste>>")
        )
        ...
        ttk.Button(
            ...
            command=lambda: path_var.set(
                filedialog.askdirectory(initialdir="C:\\") or path_var.get()
            ),
        ).pack(side=LEFT)

        ttk.Label(
            frame,
            text="⚠️ 请输入C盘下的有效目录路径",
            font=("微软雅黑", 8),
            foreground="#888",
        ).pack(anchor=W)
        ...
        ttk.Button(
            btn_frame, text="添加", command=do_add, bootstyle="success", width=10
        ).pack(side=RIGHT, padx=(5, 0))
        ttk.Button(
            btn_frame,
            text="取消",
    ...
    def _save(self):
        ...
        if has_changes:
            ...
            if result is True:
                ...
                messagebox.showinfo(
                    "提示", "设置已保存，稍后可手动重建C盘索引", parent=self.dialog
                )
                self.dialog.destroy()
        else:
...
class BatchRenameDialog:
    ...
    def show(self, scope_text=""):
        ...
        ttk.Label(main, text=scope_text, font=("微软雅黑", 9), foreground="#555").pack(
            anchor=W, pady=(0, 5)
        )
        ...
        ttk.Entry(prefix_row, textvariable=self.prefix_var, width=20).pack(
            side=LEFT, padx=(3, 15)
        )

        ttk.Label(prefix_row, text="起始序号:").pack(side=LEFT)
        ttk.Entry(prefix_row, textvariable=self.start_num_var, width=6).pack(
            side=LEFT, padx=(3, 15)
        )

        ttk.Label(prefix_row, text="序号位数:").pack(side=LEFT)
        ttk.Entry(prefix_row, textvariable=self.width_var, width=4).pack(
            side=LEFT, padx=(3, 0)
        )
        ...
        ttk.Entry(replace_row, textvariable=self.find_var, width=18).pack(
            side=LEFT, padx=(3, 15)
        )

        ttk.Label(replace_row, text="替换为:").pack(side=LEFT)
        ttk.Entry(replace_row, textvariable=self.replace_var, width=18).pack(
            side=LEFT, padx=(3, 0)
        )
        ...
        self.preview_text = tk.Text(
            preview_frame, font=("Consolas", 9), height=12, wrap="none"
        )
        ...
        ttk.Button(
            btn_frame, text="预览效果", command=self._update_preview, bootstyle="info"
        ).pack(side=LEFT)

        ttk.Button(
            btn_frame, text="执行重命名", command=self._do_rename, bootstyle="success"
        ).pack(side=LEFT, padx=8)

        ttk.Button(
            btn_frame, text="关闭", command=self.dialog.destroy, bootstyle="secondary"
        ).pack(side=RIGHT)
...
class SearchApp:
    ...
    def _build_menubar(self):
        ...
        file_menu.add_command(
            label="📤 导出结果", command=self.export_results, accelerator="Ctrl+E"
        )
        file_menu.add_separator()
        file_menu.add_command(
            label="📂 打开文件", command=self.open_file, accelerator="Enter"
        )
        file_menu.add_command(
            label="🎯 定位文件", command=self.open_folder, accelerator="Ctrl+L"
        )
        file_menu.add_separator()
        file_menu.add_command(
            label="🚪 退出", command=self._do_quit, accelerator="Alt+F4"
        )
        ...
        edit_menu.add_command(
            label="📋 复制路径", command=self.copy_path, accelerator="Ctrl+C"
        )
        edit_menu.add_command(
            label="📄 复制文件", command=self.copy_file, accelerator="Ctrl+Shift+C"
        )
        edit_menu.add_separator()
        edit_menu.add_command(
            label="🗑️ 删除", command=self.delete_file, accelerator="Delete"
        )
        ...
        search_menu.add_command(
            label="🔍 开始搜索", command=self.start_search, accelerator="Enter"
        )
        search_menu.add_command(
            label="🔄 刷新搜索", command=self.refresh_search, accelerator="F5"
        )
        search_menu.add_command(
            label="⏹ 停止搜索", command=self.stop_search, accelerator="Escape"
        )
        ...
        tool_menu.add_command(
            label="📊 大文件扫描", command=self.scan_large_files, accelerator="Ctrl+G"
        )
    ...
    def _update_favorites_menu(self):
        ...
        self.fav_menu.add_command(
            label="⭐ 收藏当前目录", command=self._add_current_to_favorites
        )
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            row0, text="🎯 增强版", font=("微软雅黑", 10), foreground="#FF9800"
        ...
        ttk.Label(row0, text="主题:", font=("微软雅黑", 9)).pack(
        ...
        ttk.Button(
            row0,
            text="📂 C盘目录",
            command=self._show_c_drive_settings,
            bootstyle="warning-outline",
            width=10,
        ).pack(side=RIGHT, padx=2)
        ttk.Button(
        ...
        ttk.Button(
            row0,
            text="🔄 刷新状态",
            command=self.refresh_index_status,
            bootstyle="info-outline",
            width=10,
        ).pack(side=RIGHT, padx=2)
        ...
        self.combo_fav = ttk.Combobox(
            row1, textvariable=self.fav_combo_var, state="readonly", width=10
        )
        ...
        self.entry_kw = ttk.Entry(
            row1, textvariable=self.kw_var, font=("微软雅黑", 12), width=40
        )
        ...
        # 搜索选项
        ttk.Checkbutton(
            row1, text="模糊", variable=self.fuzzy_var, bootstyle="round-toggle"
        ).pack(side=LEFT, padx=(0, 3))
        ttk.Checkbutton(
            row1, text="正则", variable=self.regex_var, bootstyle="round-toggle"
        ).pack(side=LEFT, padx=(0, 5))
        ttk.Checkbutton(
            row1, text="实时", variable=self.force_realtime, bootstyle="round-toggle"
        ...
        self.combo_ext = ttk.Combobox(
            row2, textvariable=self.ext_var, state="readonly", width=15, values=["全部"]
        )
        ...
        self.lbl_filter = ttk.Label(
            row2, text="", font=("微软雅黑", 9), foreground="#666"
        )
        ...
        for col, text, w in zip(
            ...
            self.tree.heading(
                col, text=text, command=lambda c=col: self.sort_column(c, False)
            )
            self.tree.column(
                col, width=w, anchor="w" if col in ("filename", "path") else "center"
            )
        ...
        self.btn_first = ttk.Button(
            pg_ctr,
            text="⏮",
            command=lambda: self.go_page("first"),
            bootstyle="link",
            state="disabled",
        )
        ...
        self.btn_prev = ttk.Button(
            pg_ctr,
            text="◀",
            command=lambda: self.go_page("prev"),
            bootstyle="link",
            state="disabled",
        )
        ...
        self.btn_next = ttk.Button(
            pg_ctr,
            text="▶",
            command=lambda: self.go_page("next"),
            bootstyle="link",
            state="disabled",
        )
        ...
        self.btn_last = ttk.Button(
            pg_ctr,
            text="⏭",
            command=lambda: self.go_page("last"),
            bootstyle="link",
            state="disabled",
        )
        ...
        ttk.Label(btm, textvariable=self.status, font=("微软雅黑", 9)).pack(
            side=LEFT, padx=10
        )
        self.status_path = tk.StringVar()
        ttk.Label(
            btm,
            textvariable=self.status_path,
            font=("Consolas", 8),
            foreground="#718096",
        ).pack(side=LEFT, fill=X, expand=True)
        self.progress = ttk.Progressbar(
            btm, mode="indeterminate", bootstyle="success", length=200
        )
    ...
    def _bind_shortcuts(self):
        ...
        self.root.bind(
            "<Escape>",
            lambda e: self.stop_search() if self.is_searching else self.kw_var.set(""),
        )
    ...
    def _show_c_drive_settings(self):
        dialog = CDriveSettingsDialog(
            self.root, self.config_mgr, self.index_mgr, self._rebuild_c_drive
        )
        dialog.show()
    ...
    def _filter_by_drive(self, drive_path):
        ...
        self.lbl_filter.config(
            text=f"磁盘筛选: {len(self.filtered_results)}/{len(self.full_search_results)}"
        )
    ...
    def _show_settings(self):
        ...
        ttk.Label(frame, text="常规设置", font=("微软雅黑", 12, "bold")).pack(
            anchor=W, pady=(0, 15)
        )
        ...
        if not HAS_WIN32:
            ttk.Label(hotkey_frame, text="(需要pywin32)", foreground="gray").pack(
                side=LEFT, padx=10
            )
        ...
        if not HAS_TRAY:
            ttk.Label(tray_frame, text="(需要pystray和PIL)", foreground="gray").pack(
                side=LEFT, padx=10
            )
        ...
        ttk.Button(
            btn_frame, text="保存", command=save_settings, bootstyle="success", width=10
        ).pack(side=RIGHT, padx=(5, 0))
        ttk.Button(
            btn_frame, text="取消", command=dlg.destroy, bootstyle="secondary", width=10
        ).pack(side=RIGHT)
    ...
    def _manage_favorites(self):
        ...
        ttk.Label(frame, text="收藏夹列表", font=("微软雅黑", 11, "bold")).pack(
            anchor=W
        )
        ...
        ttk.Button(
            btn_frame, text="关闭", command=dlg.destroy, bootstyle="secondary"
        ...
        if history:
            for kw in history[:15]:
                self.history_menu.add_command(
                    label=kw,
                    command=lambda k=kw: (self.kw_var.set(k), self.start_search()),
                )
    ...
    def _get_search_scope_targets(self):
        return parse_search_scope(
            self.scope_var.get(), self._get_drives, self.config_mgr
        )
  ...
  Ctrl+Shift+Tab    主窗口
    ...
    def _apply_filter(self):
        ...
        if "磁盘筛选" in current_filter_text:
            ...
            if ext_sel != "全部" or size_min > 0 or date_min > 0:
                self.lbl_filter.config(
                    text=f"{base_text} | 筛选: {filtered_count}/{all_count}"
                )
            else:
    ...
    def _update_page_info(self):
        ...
        self.lbl_page.config(
            text=f"第 {self.current_page}/{self.total_pages} 页 ({total}项)"
        )
    ...
    def _get_sel(self):
        ...
        if not sel:
            return None
        first_sel = sel[0]
        if first_sel not in self.item_meta:
            return None
        idx = self.item_meta[first_sel]
        with self.results_lock:
    ...
    def start_search(self):
        ...
        if use_idx:
            ...
            threading.Thread(
                target=self._search_idx,
                args=(self.current_search_id, keywords, scope_targets),
                daemon=True,
            ).start()
        else:
            ...
            threading.Thread(
                target=self._search_rt,
                args=(self.current_search_id, kw, scope_targets),
                daemon=True,
            ).start()
                                    ...
                                    if check(name):
                                        local_batch.append(
                                            (
                                                ...
                                                st.st_size,
                                                st.st_mtime,
                                                1 if ext in ARCHIVE_EXTS else 2,
                                            )
                                        )
        ...
        try:
            for _ in range(200):
                ...
                if t == "BATCH":
                    ...
                    self.status.set(
                        f"🔍 实时扫描中... (已扫描 {scanned_dirs:,} 个目录，{speed:.0f}/s)"
                    )
                elif t == "IDX_PROG":
                    self._check_index()
    ...
    def _show_index_mgr(self):
        ...
        for i, (l, v) in enumerate(rows):
            ...
            ttk.Label(
                info, text=v, foreground="#28a745" if "✅" in str(v) else "#555"
        ...
        ttk.Button(
            bf, text="🔄 重建索引", command=rebuild, bootstyle="primary", width=14
        ).grid(row=0, column=0, padx=5)
        ttk.Button(
            bf, text="📁 更改位置", command=browse, bootstyle="secondary", width=14
        ).grid(row=0, column=1, padx=5)
        ttk.Button(
            bf, text="🗑️ 删除索引", command=delete, bootstyle="danger-outline", width=14
    ...
    def export_results(self):
        ...
        path = filedialog.asksaveasfilename(
            title="导出结果", defaultextension=".csv", filetypes=filetypes
        )
        if not path:
            return
    ...
    def scan_large_files(self):
        ...
        self.lbl_filter.config(
            text=f"大文件: {len(large_files)}/{len(self.all_results)}"
        )
    ...
    def find_empty_folders(self):
        ...
        with self.results_lock:
            for item in self.all_results:
                if item["type_code"] == 0:
                    try:
                        if os.path.exists(item["fullpath"]) and not os.listdir(
                            item["fullpath"]
                        ):
        ...
        self.lbl_filter.config(
            text=f"空文件夹: {len(empty_folders)}/{len(self.all_results)}"
        )
