...
if IS_WINDOWS:
    ...
    def enum_volume_files_mft(drive_letter, skip_dirs, skip_exts, allowed_paths=None):
        ...
        # 调试日志
        logger.info(f"[DEBUG] drive={drive_letter}, allowed_paths={allowed_paths}")
        ...
        try:
            ...
            if all_drives and IS_WINDOWS:
                ...
                if all_data:
                    ...
                    with self.lock:
                        self.conn.cursor().execute("PRAGMA synchronous=OFF; PRAGMA journal_mode=MEMORY;")
                    with self.lock, self.conn:
                        self.conn.cursor().executemany("INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)", all_data)
                    with self.lock:
                        self.conn.cursor().execute("PRAGMA synchronous=NORMAL; PRAGMA journal_mode=WAL;")
    ...
    def rebuild_drive(self, drive_letter, progress_cb=None, stop_fn=None):
        ...
        try:
            with self.lock, self.conn:
                ...
                self.conn.cursor().execute("DELETE FROM files WHERE full_path LIKE ?", (drive_pattern,))
            ...
            if IS_WINDOWS:
                try:
                    ...
                    data = enum_volume_files_mft(drive, SKIP_DIRS_LOWER, SKIP_EXTS, allowed_paths=(scan_paths if drive == 'C' else None))
                    data_to_write.extend(data)
                    if progress_cb: progress_cb(len(data), f"MFT {drive}:")
                except Exception as e:
                    logger.warning(f"[MFT] {drive}: 失败，使用传统扫描 - {e}")
                    for path in scan_paths: self._scan_dir(path, (scan_paths if drive == 'C' else None), progress_cb, stop_fn)
            else:
            ...
            if data_to_write:
                ...
                with self.lock, self.conn:
                    self.conn.cursor().executemany("INSERT OR IGNORE INTO files VALUES(NULL,?,?,?,?,?,?,?,?)", data_to_write)
