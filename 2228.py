...
class FileWatcher:
    """文件监控器 - 使用 IndexManager 的连接避免锁冲突"""

    def __init__(self, mgr, config_mgr=None):
        self.mgr = mgr  # IndexManager
        self.config_mgr = config_mgr
        self.observer = None
        self.running = False
        self.eq = queue.Queue()
    ...
    def _apply(self, events):
        """应用文件变更到数据库 - 使用 IndexManager 的连接"""
        if not events or not self.mgr.conn:
            return

        # 等待索引构建完成
        if self.mgr.is_building:
            logger.debug("[监控] 索引正在构建，跳过本批次更新")
            return
        ...
        # ★ 使用 IndexManager 的锁和连接
        max_retries = 3
        for attempt in range(max_retries):
            try:
                with self.mgr.lock:
                    cursor = self.mgr.conn.cursor()
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"[监控] 更新重试 {attempt + 1}/{max_retries}: {e}")
                    time.sleep(0.5 * (attempt + 1))
                else:
                    logger.error(f"[监控] 数据库更新失败: {e}")
