...
// 极速文件搜索 V52 - Arc + free对称 + path_map优化 + seen压缩key
// 2025-12-20
...
use windows_sys::Win32::{
    ...
    Storage::FileSystem::{
        CreateFileW, FILE_FLAG_BACKUP_SEMANTICS, FILE_FLAG_SEQUENTIAL_SCAN,
        FILE_SHARE_DELETE, FILE_SHARE_READ, FILE_SHARE_WRITE, OPEN_EXISTING,
    },
    System::IO::DeviceIoControl,
};

// GENERIC_READ 不在 FileSystem 模块，手动定义
...
/// 给主程序（索引补全/懒加载）复用的快速文件信息接口。
/// - Windows: 使用 `GetFileAttributesExW`，不需要打开文件句柄，开销很低
/// - 返回: (size, mtime_unix_seconds)
pub fn get_file_info_fast_native(path: &str) -> Option<(u64, f64)> {
    get_file_info_fast(path)
}

fn get_path_by_file_ref_with_handle(volume_handle: HANDLE, file_ref: u64) -> Option<String> {
    unsafe {
        ...
        let len = GetFinalPathNameByHandleW(file_h, path_buf.as_mut_ptr(), 520, 0);
...
struct MftRecord {
    ...
    is_dir: bool,
    file_ref: u64,
    // 来自 USN_RECORD_V2::time_stamp（FILETIME, 100ns since 1601），转换后的 unix seconds。
    // 用于索引端 dm: 过滤；无需逐文件 stat，几乎不增加索引构建开销。
    mtime: f64,
}
...
) -> Result<Arc<FxHashMap<u64, String>>, Box<dyn std::error::Error>> {
    {
        let cache = DIR_CACHE.read();
        if let Some(dc) = cache.get(&drive) {
...
fn build_path_map(records: &[MftRecord], drive: char) -> FxHashMap<u64, String> {
            ...
            if noff + nlen <= br as usize && nlen > 0 {
                ...
                if is_dir {
                    ...
                    if let Ok(name) = String::from_utf16(slice) {
                        let fc = name.as_bytes().first().copied().unwrap_or(b'.');
                        if fc != b'.' && fc != b'$' {
                            let ts_ft = if rec.time_stamp > 0 {
                                rec.time_stamp as u64
                            } else {
                                0
                            };
                            let mtime = if ts_ft > EPOCH_DIFF {
                                (ts_ft - EPOCH_DIFF) as f64 / 10_000_000.0
                            } else {
                                0.0
                            };
...
pub fn scan_and_pack(drive: char) -> Result<(Vec<u8>, usize), Box<dyn std::error::Error>> {
    ...
    let items0: Vec<_> = records
        ...
        .filter_map(|r| {
            ...
            Some((r.filename.clone(), path, parent.clone(), ext, r.is_dir, r.mtime))
        })
        .collect();

    // 性能关键点：全盘/全量索引构建时，如果在这里逐文件查 size，会触发海量 I/O，索引构建会非常慢。
    // mtime 这里直接使用 USN_RECORD_V2::time_stamp（无需逐文件 I/O），用于索引端 dm: 过滤与排序。
    // size 仍保持 0，前端/搜索阶段再按需批量补齐（仅可见页）。
    let items = items0;
    let count = items.len();

    let total_size: usize = items
        .iter()
        .map(|(name, path, parent, ext, _, _)| {
            24 + name.len() + path.len() + parent.len() + ext.len()
        })
        .sum();

    let mut data = Vec::with_capacity(total_size);

    for (filename, path, parent, ext, is_dir, mtime) in items {
